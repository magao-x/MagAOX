...
21) Test Doxygen Link Preservation
  - When test harness indirection, fault-injection wrappers, alternate namespaces, or protected/private access would prevent Doxygen from auto-linking the real API under test add explicit Doxygen-only references to the real symbol inside the relevant `TEST_CASE`.
