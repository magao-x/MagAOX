...
struct outletController
{
   ...
   ///Setup an application configurator for a outletController
   ...
   template<class callBackMapT>
   int setupINDI( const std::string & configName,
                  callBackMapT & indiNewCallBacks
                );
   
   template<class indiDriverT>
   int updateINDI( indiDriverT * indiDriver );
...
inline
int outletController::setupConfig( appConfigurator & config )
{
...
inline 
int outletController::loadConfig( appConfigurator & config )
{
...
inline 
int outletController::setNumberOfOutlets( int numOuts )
{
...
inline 
int outletController::outletState( int outletNum )
{
...
inline
int outletController::updateOutletStates()
{
...
inline
size_t outletController::numChannels()
{
...
inline 
std::vector<size_t> outletController::channelOutlets( const std::string & channel )
{
...
inline 
std::vector<size_t> outletController::channelOnOrder( const std::string & channel )
{
...
inline 
std::vector<size_t> outletController::channelOffOrder( const std::string & channel )
{
...
inline 
std::vector<unsigned> outletController::channelOnDelays( const std::string & channel )
{
...
inline 
std::vector<unsigned> outletController::channelOffDelays( const std::string & channel )
{
...
inline 
int outletController::channelState( const std::string & channel ) 
{
...
inline 
int outletController::turnChannelOn( const std::string & channel )
{
...
inline 
int outletController::turnChannelOff( const std::string & channel )
{
   //If order is specified, get first outlet number
   size_t n = 0;
...
inline
int outletController::st_newCallBack_channels( void * app, 
                                               const pcf::IndiProperty &ipRecv
                                             )
{
   return static_cast<outletController *>(app)->newCallBack_channels(ipRecv);
}


inline
int outletController::newCallBack_channels( const pcf::IndiProperty &ipRecv )
{
   ...
   std::string state = ipRecv["state"].get<std::string>();
...
template<class callBackMapT>
int outletController::setupINDI( const std::string & configName,
                                 callBackMapT & indiNewCallBacks
                               )
{
   ...
   for(auto it = m_channels.begin(); it != m_channels.end(); ++it)
   {
      ...
      it->second.m_indiP_prop.setDevice(configName);
      ...
      auto result = indiNewCallBacks.insert( {it->first, {&it->second.m_indiP_prop, st_newCallBack_channels}});
   ...
   m_indiP_outletStates.setDevice(configName);
   ...
   auto result =  indiNewCallBacks.insert( { "outlet", {&m_indiP_outletStates, nullptr}});
...
template<class indiDriverT>
int outletController::updateINDI( indiDriverT * indiDriver )
{
   if( !indiDriver ) return 0;
   
   //Publish outlet states (only bother if they've changed)
   for(size_t i=0; i< m_outletStates.size(); ++i)
   {
      indi::updateIfChanged(m_indiP_outletStates, std::to_string(i+1), stateIntToString(m_outletStates[i]), indiDriver);
   }
   ...
   for(auto it = m_channels.begin(); it != m_channels.end(); ++it)
   {
      ...
      indi::updateIfChanged( it->second.m_indiP_prop, "state", state, indiDriver );
      indi::updateIfChanged( it->second.m_indiP_prop, "target", target, indiDriver );
   }
