...
#ifndef elliptecWheelCtrl_hpp
#define elliptecWheelCtrl_hpp


#include "../../libMagAOX/libMagAOX.hpp" //Note this is included on command line to trigger pch
#include "../../magaox_git_version.h"

/** \defgroup elliptecWheelCtrl Filter Wheel Control
  * \brief Control of MagAO-X MCBL-based f/w.
  *
  * <a href="../handbook/operating/software/apps/elliptecWheelCtrl.html">Application Documentation</a>
  *
  * \ingroup apps
  *
  */

/** \defgroup elliptecWheelCtrl_files Filter Wheel Control Files
  * \ingroup elliptecWheelCtrl
  */
...
/** MagAO-X application to control a Faulhaber MCBL controlled filter wheel.
  ...
  * \ingroup elliptecWheelCtrl
  */
class elliptecWheelCtrl : public MagAOXApp<>, public tty::usbDevice, public dev::stdMotionStage<elliptecWheelCtrl>, public dev::telemeter<elliptecWheelCtrl>
{

   friend class dev::stdMotionStage<elliptecWheelCtrl>;
   
   friend class dev::telemeter<elliptecWheelCtrl>;
...
public:
   INDI_NEWCALLBACK_DECL(elliptecWheelCtrl, m_indiP_counts);
...
inline
elliptecWheelCtrl::elliptecWheelCtrl() : MagAOXApp(MAGAOX_CURRENT_SHA1, MAGAOX_REPO_MODIFIED)
{
...
inline
void elliptecWheelCtrl::setupConfig()
{
   ...
   dev::stdMotionStage<elliptecWheelCtrl>::setupConfig(config);
   
   dev::telemeter<elliptecWheelCtrl>::setupConfig(config);
}

inline
void elliptecWheelCtrl::loadConfig()
{
   ...
   dev::stdMotionStage<elliptecWheelCtrl>::loadConfig(config);

   dev::telemeter<elliptecWheelCtrl>::loadConfig(config);
}

inline
int elliptecWheelCtrl::appStartup()
{
   ...
   dev::stdMotionStage<elliptecWheelCtrl>::appStartup();
   
   if(dev::telemeter<elliptecWheelCtrl>::appStartup() < 0)
   {
...
inline
int elliptecWheelCtrl::appLogic()
{
   ...
   if( state() == stateCodes::NOTHOMED || state() == stateCodes::READY || state() == stateCodes::OPERATING || state() == stateCodes::HOMING)
   {
      ...
      stdMotionStage<elliptecWheelCtrl>::updateINDI();
      recordStage();
      
      if(telemeter<elliptecWheelCtrl>::appLogic() < 0)
      {
         log<software_error>({__FILE__, __LINE__});
         return 0;
      }
...
inline
int elliptecWheelCtrl::appShutdown()
{
   //don't bother
   return 0;
}
...
inline
int elliptecWheelCtrl::onPowerOff()
{
   if( stdMotionStage<elliptecWheelCtrl>::onPowerOff() < 0)
   {
      log<software_error>({__FILE__,__LINE__});
   }
...
inline
int elliptecWheelCtrl::whilePowerOff()
{
   if( stdMotionStage<elliptecWheelCtrl>::whilePowerOff() < 0)
   {
      log<software_error>({__FILE__,__LINE__});
   }
...
INDI_NEWCALLBACK_DEFN(elliptecWheelCtrl, m_indiP_counts)(const pcf::IndiProperty &ipRecv)
{
...
int elliptecWheelCtrl::onPowerOnConnect()
{
   std::string com;
...
int elliptecWheelCtrl::getSwitch()
{
   int rv;
   std::string resp;
...
int elliptecWheelCtrl::getMoving()
{
   int rv;
   std::string resp;
...
int elliptecWheelCtrl::startHoming()
{
   m_homingState = 1;
   m_moving = 2;
   recordStage(true);
...
int elliptecWheelCtrl::presetNumber()
{
...
int elliptecWheelCtrl::moveToRaw( const long & counts )
{
...
int elliptecWheelCtrl::moveToRawRelative( const long & counts_relative )
{
...
int elliptecWheelCtrl::moveTo( const double & filters )
{
   long counts;
...
int elliptecWheelCtrl::checkRecordTimes()
{
   return dev::telemeter<elliptecWheelCtrl>::checkRecordTimes(telem_stage(), telem_position());
}
   
int elliptecWheelCtrl::recordTelem( const telem_stage * )
{
   return recordStage(true);
}

int elliptecWheelCtrl::recordTelem( const telem_position * )
{
   return recordPosition(true);
}

int elliptecWheelCtrl::recordStage(bool force)
{
   return dev::stdMotionStage<elliptecWheelCtrl>::recordStage(force);
}

int elliptecWheelCtrl::recordPosition(bool force)
{
