/** \file orcaUtils.hpp
 * \brief Utilities for the Hamamatsu Orca Quest2 camera controller.
 *
...
#ifndef hamamatsuUtils_hpp
#define hamamatsuUtils_hpp
...
/// Structure holding the temperature values reported by the Hamamatsu Orca Quest2.
/**
 * \ingroup orcaCtrl
 */
struct hamamatsuTemps
{
    ...
    bool operator==( const hamamatsuTemps &t /**< [in] the values to compare against */ ) const
    {
...
/// Hamamatsu Orca Quest 2 ROI expressed as 0-based inclusive column and row limits.
/**
 * \ingroup orcaCtrl
 */
struct hamamatsuRoi
...
/// Strip an optional prompt and surrounding whitespace from a Hamamatsu Orca Quest 2 CLI response.
inline std::string hamamatsuCleanResponse( const std::string &response /**< [in] raw CLI response */ )
{
...
inline int hamamatsuParseFloat( float             &value,   ///< [out] parsed floating-point value
                            const std::string &response /**< [in] raw or cleaned CLI response */
)
{
    std::string clean = hamamatsuCleanResponse( response );
    if( clean.empty() )
    {
        return -1;
    }
...
inline int hamamatsuParseFloatVector( std::vector<float> &values,        ///< [out] parsed floating-point values
                                  const std::string  &response,      /**< [in] raw or cleaned CLI response */
...
)
{
    ...
    for( const auto &token : tokens )
    {
        ...
        if( hamamatsuParseFloat( value, token ) < 0 )
        {
            values.clear();
            return -1;
        }
...
inline int hamamatsuParseBool( bool              &value,   ///< [out] parsed boolean value
                           const std::string &response /**< [in] raw or cleaned CLI response */
)
{
    std::string clean = hamamatsuCleanResponse( response );
...
inline int hamamatsuParseRange( int               &firstValue,  ///< [out] first parsed range value
                            int               &secondValue, ///< [out] second parsed range value
                            const std::string &response     /**< [in] raw or cleaned CLI response */
);

/// Parse a raw cropping status response such as `on` or `on:192-447:128-383`.
inline int hamamatsuParseCropState( bool              &enabled,     ///< [out] parsed cropping-enabled flag
...
)
{
    ...
    if( hamamatsuParseBool( enabled, tokens[0] ) < 0 )
    {
        return -1;
    }
    ...
    if( hamamatsuParseRange( startColumn, endColumn, tokens[1] ) < 0 || hamamatsuParseRange( startRow, endRow, tokens[2] ) < 0 )
    {
        return -1;
    }
...
inline bool hamamatsuResponseOK( const std::string &response /**< [in] raw or cleaned CLI response */ )
{
    std::string clean = hamamatsuCleanResponse( response );
...
/// Convert a MagAO-X ROI center/size description into Hamamatsu Orca Quest 2 corners.
inline int hamamatsuRoiFromCenter( hamamatsuRoi &roi,       ///< [out] the corresponding Hamamatsu Orca Quest 2 ROI
...
/// Convert Hamamatsu Orca Quest 2 ROI corners into a MagAO-X ROI center/size description.
inline int hamamatsuRoiToCenter( float          &centerX,   ///< [out] ROI x center coordinate
                             float          &centerY,   ///< [out] ROI y center coordinate
                             int            &width,     ///< [out] ROI width in pixels
                             int            &height,    ///< [out] ROI height in pixels
                             const hamamatsuRoi &roi,       /**< [in] the Hamamatsu Orca Quest 2 ROI to convert */
                             int             fullWidth, /**< [in] detector full-frame width */
                             int             fullHeight /**< [in] detector full-frame height */
)
{
...
inline std::string hamamatsuColumnsSpec( const hamamatsuRoi &roi /**< [in] the ROI to format */ )
{
    return std::to_string( roi.startColumn ) + "-" + std::to_string( roi.endColumn );
}

/// Format the row command payload for `set cropping rows`.
inline std::string hamamatsuRowsSpec( const hamamatsuRoi &roi /**< [in] the ROI to format */ )
{
