...
/** \defgroup orcaCtrl Orca Camera
 * \brief Control of the Hamamatsu Orca camera.
 *
                  ...
                  public dev::telemeter<orcaCtrl>
{
  ...
  public:
    ...
    static constexpr bool c_stdCamera_exptimeCtrl  = true; ///< Expose exposure-time controls.
  ...
  protected:
    ...
    // TODO: will probably need to revise this for orca pci card
    int m_serialBaud{ 115200 }; ///< Camera Link serial baud rate used for Orca CLI access.
    ///@}

    /** \name Orca State - Data
     * @{
     */
    orcaTemps m_temps; ///< Cached camera temperature values used for INDI and telemetry updates.
     ...
     * @{
     */
    pcf::IndiProperty m_indiP_temps;     ///< Property reporting the detailed Orca temperature channels.
    pcf::IndiProperty m_indiP_fpsLimits; ///< Property reporting the current Orca minimum and maximum FPS.
  ...
  public:
    ...
    /// Implement the Orca temperature-controller toggle semantics.
    int setTempControl();
    ...
    /// Required by `stdCamera`, but unused for Orca.
    int setExpTime();
    ...
    /// Record the detailed Orca temperature telemetry.
    int recordTelem( const orca_temps * /**< [in] type-dispatch tag */ );
    ...
    /// Record the detailed Orca temperature telemetry when values change.
...
/// Normalize a Orca text response for tolerant string parsing.
inline std::string orcaLowerResponse( const std::string &response /**< [in] raw or cleaned CLI response */ )
{
    std::string clean = orcaCleanResponse( response );
...
/// Convert a Orca fan percentage into the nearest exposed preset name.
inline std::string orcaFanPresetName( float fanPercent /**< [in] current or requested fan percentage */ )
{
    if( fanPercent <= 0.5f )
    {
        return "off";
...
inline int orcaFanPresetPercent( int               &fanPercent, ///< [out] mapped manual fan percentage
...
/// Parse a Orca sensibility response into the exposed analog-gain preset name.
inline int orcaAnalogGainName( std::string       &gainName, ///< [out] exposed analog-gain preset name
                                const std::string &response  /**< [in] raw or cleaned sensibility response */
)
{
    std::string clean = orcaLowerResponse( response );
...
/// Convert an exposed analog-gain preset name into the Orca command argument.
inline int orcaAnalogGainCommand( std::string       &commandGain, ///< [out] Orca sensibility command argument
...
inline void orcaCtrl::setupConfig()
{
    ...
    dev::edtCamera<orcaCtrl>::setupConfig( config );
    ...
    config.add( "camera.serialBaud",
                ...
                false,
                "int",
                "The Camera Link serial baud rate for Orca CLI commands. Default is 115200." );
}

inline int orcaCtrl::loadConfigImpl( mx::app::appConfigurator &config )
{
    STDCAMERA_LOAD_CONFIG( config );

    config( m_serialBaud, "camera.serialBaud" );

    m_configFile = "/tmp/orca_" + configName() + ".cfg";
    ...
    if( writeConfig() < 0 )
    {
        return log<software_critical, -1>( { __FILE__, __LINE__, "could not write initial Orca EDT config" } );
    }

    dev::edtCamera<orcaCtrl>::loadConfig( config );
...
inline int orcaCtrl::appStartup()
{
    ...
    if( dev::edtCamera<orcaCtrl>::appStartup() < 0 )
    {
        return log<software_critical, -1>( { __FILE__, __LINE__ } );
    }
...
inline int orcaCtrl::appLogic()
{
    ...
    if( dev::edtCamera<orcaCtrl>::appLogic() < 0 )
    {
    ...
    if( state() == stateCodes::NOTCONNECTED || state() == stateCodes::NODEVICE || state() == stateCodes::ERROR )
    {
        ...
        {
            float fpsValue = 0;
            if( orcaParseFloat( fpsValue, response ) == 0 )
            {
    ...
    if( state() == stateCodes::READY || state() == stateCodes::OPERATING )
    {
        ...
        if( edtCamera<orcaCtrl>::updateINDI() < 0 )
        {
            log<software_error>( { __FILE__, __LINE__ } );
            state( stateCodes::ERROR );
            return 0;
...
inline int orcaCtrl::onPowerOff()
{
    ...
    if( edtCamera<orcaCtrl>::onPowerOff() < 0 )
    {
        log<software_error>( { __FILE__, __LINE__ } );
    }
...
inline int orcaCtrl::whilePowerOff()
{
    ...
    if( stdCamera<orcaCtrl>::whilePowerOff() < 0 )
    {
        log<software_error>( { __FILE__, __LINE__ } );
    }

    if( edtCamera<orcaCtrl>::whilePowerOff() < 0 )
    {
        log<software_error>( { __FILE__, __LINE__ } );
    }
...
inline int orcaCtrl::appShutdown()
{
    ...
    dev::edtCamera<orcaCtrl>::appShutdown();
...
inline int orcaCtrl::sendCommand( std::string &response, const std::string &command, bool logFailure )
{
    ...
    { // mutex scope
        ...
        if( pdvSerialWriteRead( rawResponse, command, logFailure ) != 0 )
        {
            ...
            return log<software_error, -1>( { __FILE__, __LINE__, "error sending Orca command: " + command } );
        }
    }

    response = orcaCleanResponse( rawResponse );

    return 0;
}

inline int orcaCtrl::issueCommand( const std::string &command, bool allowNoResponse )
{
    std::string response;
    ...
    if( !orcaResponseOK( response ) )
    {
        return log<text_log, -1>( "Orca rejected command '" + command + "' with response: " + response,
                                  logPrio::LOG_ERROR );
    }
...
inline int orcaCtrl::syncROIFromCamera()
{
    std::string response;
    ...
    if( sendCommand( response, "cropping raw", false ) < 0 ||
        orcaParseCropState( cropEnabled, startColumn, endColumn, startRow, endRow, response ) < 0 )
    {
    ...
    else
    {
        ...
        if( startColumn == 0 && endColumn == 0 && startRow == 0 && endRow == 0 )
        {
            if( sendCommand( response, "cropping columns raw", false ) < 0 ||
                orcaParseRange( cameraROI.startColumn, cameraROI.endColumn, response ) < 0 )
            {
            ...
            if( sendCommand( response, "cropping rows raw", false ) < 0 ||
                orcaParseRange( cameraROI.startRow, cameraROI.endRow, response ) < 0 )
            {
                return log<software_error, -1>(
    ...
    if( m_currentROI.w != m_raw_width || m_currentROI.h != m_raw_height )
    {
        ...
        if( dev::edtCamera<orcaCtrl>::pdvReconfig() < 0 )
        {
            return log<software_error, -1>( { __FILE__, __LINE__ } );
        }
...
inline int orcaCtrl::setSerialBaud()
{
    ...
    {
        return log<software_error, -1>(
            { __FILE__, __LINE__, "failed to set Orca serial baud to " + std::to_string( m_serialBaud ) } );
    }
...
inline int orcaCtrl::getTemps()
{
    ...
    {
        return log<text_log, 0>( "transient Orca temperature refresh failure; keeping previous cached values: " +
                                     detail,
                                 logPrio::LOG_WARNING );
    };
    ...
    if( orcaParseFloatVector( bundledTemps, response, 6 ) < 0 )
    {
    ...
    if( orcaParseFloat( temps.setpoint, response ) < 0 )
    {
...
inline int orcaCtrl::getFanSpeed()
{
    ...
    fanMode = orcaLowerResponse( response );
    ...
    if( orcaParseFloat( fanPercent, response ) < 0 )
    {
        if( sendCommand( response, "fan speed" ) < 0 || orcaParseFloat( fanPercent, response ) < 0 )
        {
            return log<software_error, -1>( { __FILE__, __LINE__, "failed to parse fan speed response: " + response } );
        }
    }

    m_fanSpeedName    = orcaFanPresetName( fanPercent );
    m_fanSpeedNameSet = m_fanSpeedName;
    m_fanSpeedValid   = true;
    recordCamera();
...
inline int orcaCtrl::getAnalogGain()
{
    std::string response;
    ...
    if( orcaAnalogGainName( analogGain, response ) < 0 )
    {
...
inline int orcaCtrl::getLEDState()
{
    std::string response;
    bool        ledState = false;
    ...
    if( orcaParseBool( ledState, response ) < 0 )
    {
        std::string clean = orcaLowerResponse( response );
        ...
        else
        {
            ...
            clean = orcaLowerResponse( response );
            if( clean.find( "off" ) != std::string::npos )
            {
                ledState = false;
            }
...
inline int orcaCtrl::updateFPSLimits()
{
    std::string response;
    float       minFPS = 0;
    float       maxFPS = 0;

    if( sendCommand( response, "minfps raw" ) < 0 || orcaParseFloat( minFPS, response ) < 0 )
    {
        return log<software_error, -1>( { __FILE__, __LINE__, "failed to parse minfps response: " + response } );
    }

    if( sendCommand( response, "maxfps raw" ) < 0 || orcaParseFloat( maxFPS, response ) < 0 )
    {
    ...
    if( m_tempControlStatusSet )
    {
        if( m_ccdTempSetpt >= 19.5 )
        {
            return log<text_log, 0>(
                "temperature control is setpoint-driven for Orca; choose a target below 20 C to cool",
                logPrio::LOG_NOTICE );
        }
...
inline int orcaCtrl::setFanSpeed()
{
    ...
    {
        int fanPercent = 0;
        if( orcaFanPresetPercent( fanPercent, m_fanSpeedNameSet ) < 0 )
        {
...
inline int orcaCtrl::setAnalogGain()
{
    std::string commandGain;

    if( orcaAnalogGainCommand( commandGain, m_analogGainNameSet ) < 0 )
    {
...
inline int orcaCtrl::writeConfig()
{
    std::ofstream fout( m_configFile );
    if( fout.fail() )
    {
        return log<software_error, -1>( { __FILE__, __LINE__, "error opening Orca config file for writing" } );
    }
...
inline int orcaCtrl::configureAcquisition()
{
    ...
    orcaRoi roi;
    if( orcaRoiFromCenter( roi, m_nextROI.x, m_nextROI.y, m_nextROI.w, m_nextROI.h, m_full_w, m_full_h ) < 0 )
    {
        state( stateCodes::ERROR );
        return log<software_error, -1>( { __FILE__, __LINE__, "invalid ROI specified for Orca configure" } );
    }
    ...
    {
        if( issueCommand( "set cropping columns " + orcaColumnsSpec( roi ), true ) < 0 ||
            issueCommand( "set cropping rows " + orcaRowsSpec( roi ), true ) < 0 ||
...
inline int orcaCtrl::startAcquisition()
{
    ...
    recordCamera();
    return edtCamera<orcaCtrl>::pdvStartAcquisition();
}

inline int orcaCtrl::acquireAndCheckValid()
{
    return edtCamera<orcaCtrl>::pdvAcquire( m_currImageTimestamp );
}

inline int orcaCtrl::loadImageIntoStream( void *dest )
{
    if( frameGrabber<orcaCtrl>::loadImageIntoStreamCopy( dest, m_image_p, m_width, m_height, m_typeSize ) == nullptr )
    {
        return -1;
    }
...
inline int orcaCtrl::reconfig()
{
    ...
    std::lock_guard<std::recursive_mutex> guard( m_cameraMutex );
    int                                   rv = edtCamera<orcaCtrl>::pdvReconfig();
    if( rv < 0 )
    {
        return rv;
    }
...
inline int orcaCtrl::checkRecordTimes()
{
    return telemeter<orcaCtrl>::checkRecordTimes( orca_temps(), telem_stdcam(), telem_fgtimings() );
}

inline int orcaCtrl::recordTelem( const orca_temps * )
{
    return recordTemps( true );
}

inline int orcaCtrl::recordTelem( const telem_stdcam * )
{
    return recordCamera( true );
}

inline int orcaCtrl::recordTelem( const telem_fgtimings * )
{
    return recordFGTimings( true );
}

inline int orcaCtrl::recordTemps( bool force )
{
    static orcaTemps lastTemps;

    if( !( lastTemps == m_temps ) || force )
    {
        telem<orca_temps>( { m_temps.motherboard,
                              m_temps.frontend,
                              m_temps.powerboard,
                              m_temps.snake,
                              m_temps.setpoint,
