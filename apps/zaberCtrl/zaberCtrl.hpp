...
class zaberCtrl : public MagAOXApp<>, public dev::stdMotionStage<zaberCtrl>, public dev::telemeter<zaberCtrl>
{
  ...
  public:
    ...
    /**  \todo this actually should move to a preset position by nearest integet..  A moveToMM should be used for the mm
     * command.
     *
     * \returns 0 on success.
     * \returns -1 on error.
...
void zaberCtrl::setupConfig()
{
    config.add( "stage.lowLevelName",
                ...
                false,
                "string",
                "The name of the low-level control process actually controlling this stage.  "
                "Default is zaberLowLevel." );

    config.add( "stage.stageName",
                "",
                "stage.stageName",
                argType::Required,
                "stage",
                ...
                false,
                "string",
                "the name of this stage in the low-level process INDI properties.  "
                "Default is the configuration name." );

    config.add( "stage.countsPerMillimeter",
                "",
                ...
                false,
                "float",
                "The counts per mm calibration of the stage.  Default is 10078.74, "
                "which applies tot he X-LRQ-DE." );

    config.add( "stage.maxCounts",
                "",
                "stage.maxCounts",
                argType::Required,
                "stage",
...
INDI_SETCALLBACK_DEFN( zaberCtrl, m_indiP_stageMaxRawPos )( const pcf::IndiProperty &ipRecv )
{
    ...
    if( maxRawPos > 10000000000000 || maxRawPos < 0 )
        return 0; // Indicates a bad value

    if( (unsigned long)maxRawPos != m_maxRawPos &&
        ( state() == stateCodes::CONNECTED || state() == stateCodes::HOMING || state() == stateCodes::READY ||
          state() == stateCodes::OPERATING ) )
    {
        log<text_log>( std::format( "max position mismatch between config-ed value ({}) and stage value ({})",
                                    m_maxRawPos,
                                    maxRawPos ),
                       logPrio::LOG_WARNING );
    }
