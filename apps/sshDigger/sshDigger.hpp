...
int sshDigger::loadConfigImpl( mx::app::appConfigurator &_config )
{
    ...
    for( size_t i = 0; i < sections.size(); ++i )
    {
        ...
        if( rh == "" )
        {
            log<text_log>( "Config section [" + sections[i] +
...
inline void sshDigger::_sshLogThreadStart( sshDigger *s )
{
    s->sshLogThreadExec();
}

inline int sshDigger::sshLogThreadStart()
{
    try
    {
...
inline void sshDigger::sshLogThreadExec()
{
    char buffer[4096];
...
inline void sshDigger::_autosshLogThreadStart( sshDigger *s )
{
    s->autosshLogThreadExec();
}

inline int sshDigger::autosshLogThreadStart()
{
    try
    {
...
inline void sshDigger::autosshLogThreadExec()
{
    char buffer[4096];
...
inline int sshDigger::processAutoSSHLog( const std::string &logs )
{
