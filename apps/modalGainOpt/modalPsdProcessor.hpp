...
/// continuum shaping.
template <typename realT> class modalPsdProcessor {
public:
  ...
  static mx::error_t analyzePsd(
      processResults &result, /**< [out] the populated process-model results */
      const std::vector<realT>
          &measuredPsd,               /**< [in] the measured one-sided PSD */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
      size_t modeIndex,               /**< [in] the zero-based mode index */
      const processModelConfig
          &config, /**< [in] the disturbance-PSD configuration */
      realT lpContinuumFreq =
          static_cast<realT>(0), /**< [in] the LP continuum cutoff */
      realT lpContinuumWidthHz =
          c_defaultLpContinuumWidthHz, /**< [in] LP smoothing width */
      const std::vector<realT> *etfPsd =
          nullptr, /**< [in] optional CL ETF^2 correction */
      const std::vector<realT> *ntfPsd =
          nullptr /**< [in] optional CL NTF^2 correction */
  );
  ...
  static mx::error_t estimateNoisePsd(
      ...
      std::string noiseEstimateRange =
          c_defaultNoiseEstimateRange, /**< [in] which PSD end to use */
      std::string noiseEstimateStatistic =
          c_defaultNoiseEstimateStatistic, /**< [in] how to summarize the
                                              selected bins */
      realT noiseEstimateLowFreqMaxHz =
          c_defaultNoiseEstimateLowFreqMaxHz /**< [in] optional low-frequency
                                                upper limit */
  );

  /// Replace all LP content above a cutoff with a smoothed continuum.
  static mx::error_t applyLpContinuum(
      std::vector<realT> &lpProcessPsd, /**< [out] the LP disturbance PSD */
      const std::vector<realT>
          &processPsd,                /**< [in] the nominal disturbance PSD */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
...
protected:
  ...
  static size_t firstPositiveFreqIndex(
      const std::vector<realT> &freq /**< [in] the one-sided frequency grid */);

  /// Normalize a noise-estimation-domain name to lowercase hyphenated form.
  static std::string normalizeNoiseEstimateDomain(
      std::string domain /**< [in] the requested domain name */);

  /// Normalize a noise-estimation-range name to lowercase hyphenated form.
  static std::string normalizeNoiseEstimateRange(
      std::string range /**< [in] the requested PSD-end selector */);

  /// Normalize a noise-estimation-statistic name to lowercase hyphenated form.
  static std::string normalizeNoiseEstimateStatistic(
      std::string statistic /**< [in] the requested noise-fit statistic */);

  /// Normalize a CL-to-OL PSD reconstruction method name to lowercase
  /// hyphenated form.
  static std::string normalizeClosedLoopOlEstimateMethod(
      std::string method /**< [in] the requested CL-to-OL method */);

  /// Normalize a power-law crossover mode name to lowercase hyphenated form.
  static std::string normalizePowerLawCrossoverMode(
      std::string mode /**< [in] the requested crossover mode */);

  /// Resolve the power-law normalization frequency, defaulting to the first
  /// positive bin.
  static realT resolvePowerLawNormFreq(
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
  ...
  /// sampling.
  static mx::error_t buildSmoothedProcessPsd(
      std::vector<realT>
          &smoothedProcessPsd, /**< [out] the smoothed disturbance PSD */
      const std::vector<realT>
          &rawProcessPsd,             /**< [in] the raw disturbance PSD */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
  ...
  static mx::error_t findAutoPowerLawCrossoverFreq(
      ...
      const std::vector<realT>
          &smoothedProcessPsd, /**< [in] the smoothed disturbance PSD */
  ...
  static mx::error_t resolvePowerLawCrossoverFrequencies(
      ...
      const std::vector<realT>
          &rawProcessPsd, /**< [in] raw OL disturbance PSD */
      const std::vector<realT>
          &smoothedProcessPsd, /**< [in] smoothed OL disturbance PSD */
  ...
  static realT powerLawContinuum(
  ...
  static realT interpolatePsdAtFreq(
      const std::vector<realT> &psd,  /**< [in] the sampled PSD values */
  ...
  static mx::error_t matchPowerLawAtFreq(
      realT &extrapolation, /**< [in.out] the continuum normalization */
      const std::vector<realT>
          &rawProcessPsd,             /**< [in] the raw disturbance PSD */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
      realT powerLawIndex,            /**< [in] the power-law exponent */
  ...
  static realT moffatAlphaFromFwhm(
      realT fwhm, /**< [in] the desired full-width at half-maximum */
      realT beta  /**< [in] the Moffat beta parameter */
  );

  /// Evaluate a zero-background Moffat profile from its height, FWHM, and beta.
  static realT moffatValueFromFwhm(
      realT radius,     /**< [in] the distance from the peak center */
  ...
  /// a target radius.
  static realT fitMoffatBetaToBoundary(
      realT peakHeight, /**< [in] the peak height above the continuum */
      realT fwhm,       /**< [in] the full-width at half maximum */
  ...
  /// level.
  static realT
  interpolateCrossing(realT x0,    /**< [in] the first x-coordinate */
  ...
  static mx::error_t fitPowerLawIndexFromBinnedMedians(
      ...
      realT includeMatchFreqHz =
          static_cast<realT>(0) /**< [in] optional explicit match point */
  );
  ...
  static mx::error_t estimatePowerLawContinuum(
      ...
      realT powerLawFitMinFreqHz =
          c_defaultPowerLawFitMinFreqHz, /**< [in] fit low edge */
      realT powerLawFitMaxFreqHz =
          c_defaultPowerLawFitMaxFreqHz, /**< [in] fit high edge */
      realT powerLawFitBinWidthHz =
          c_defaultPowerLawFitBinWidthHz, /**< [in] fit bin width */
      bool powerLawFitIncludesMatchPoint =
          c_defaultPowerLawFitIncludesMatchPoint, /**< [in] include match point
                                                   */
      realT *usedPowerLawIndex =
          nullptr, /**< [out] the exponent actually used */
      size_t *fitBinsUsed =
          nullptr /**< [out] the number of populated fit bins */
  );

  /// Populate a sampled power-law continuum from a resolved normalization.
  static mx::error_t buildPowerLawContinuum(
      std::vector<realT> &continuumPsd, /**< [out] the sampled continuum PSD */
      realT extrapolation,              /**< [in] the continuum normalization */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
      realT powerLawIndex,            /**< [in] the power-law exponent */
      realT powerLawNormFreq          /**< [in] the normalization frequency */
  ...
  /// point.
  static mx::error_t blendContinuumAtAnchor(
      std::vector<realT> &processPsd, /**< [out] the blended disturbance PSD */
      const std::vector<realT>
          &rawProcessPsd, /**< [in] measured disturbance PSD */
      const std::vector<realT>
          &continuumPsd,  /**< [in] extrapolated continuum PSD */
  ...
  static mx::error_t identifyMoffatPeaks(
      std::vector<identifiedPeak1D> &peaks, /**< [out] the detected peaks */
      const std::vector<realT>
          &rawProcessPsd, /**< [in] the raw disturbance PSD */
      const std::vector<realT>
          &continuumPsd,              /**< [in] the continuum PSD model */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
  ...
  static mx::error_t addClippedMoffatPeakExcess(
      std::vector<realT> &peakModel, /**< [in.out] the accumulated peak model */
      const identifiedPeak1D &peak,  /**< [in] the detected peak */
      const std::vector<realT>
          &sourcePsd, /**< [in] the source PSD used to bound the peak */
  ...
  /// residual.
  static mx::error_t identifyMoffatPeaksMultiPass(
      std::vector<identifiedPeak1D> &peaks, /**< [out] the detected peaks */
      const std::vector<realT>
          &rawProcessPsd, /**< [in] the raw disturbance PSD */
      const std::vector<realT>
  ...
  static mx::error_t buildMoffatProcessFromContinuum(
      std::vector<realT> &processPsd,         /**< [out] the disturbance PSD */
  ...
  static mx::error_t buildPowerLawOnlyProcessFromContinuum(
      std::vector<realT> &processPsd,         /**< [out] the disturbance PSD */
      std::vector<unsigned char> &repairMask, /**< [out] repair-eligible bins */
      const std::vector<realT> &rawProcessPsd, /**< [in] raw disturbance PSD */
      const std::vector<realT> &noisePsd,      /**< [in] the flat noise PSD */
  ...
  static mx::error_t estimateProcessPsdPowerLawOnly(
      std::vector<realT> &processPsd, /**< [out] the disturbance PSD */
      ...
      std::vector<unsigned char>
          &repairMask, /**< [out] bins eligible for repair */
      const std::vector<realT>
          &measuredPsd, /**< [in] the measured one-sided PSD */
      const std::vector<realT> &noisePsd, /**< [in] the flat noise PSD */
      const std::vector<realT> &freq, /**< [in] the one-sided frequency grid */
  ...
  /// Moffat peaks.
  static mx::error_t estimateProcessPsdMoffatPeaks(
      std::vector<realT> &processPsd, /**< [out] the disturbance PSD */
      realT &extrapolation, /**< [out] the power-law continuum anchor */
      std::vector<identifiedPeak1D> &peaks, /**< [out] the detected peaks */
      std::vector<unsigned char>
...
mx::error_t modalPsdProcessor<realT>::analyzePsd(
    processResults &result, const std::vector<realT> &measuredPsd,
    const std::vector<realT> &freq, size_t modeIndex,
    const processModelConfig &config, realT lpContinuumFreq,
    realT lpContinuumWidthHz, const std::vector<realT> *etfPsd,
    const std::vector<realT> *ntfPsd) {
  ...
  if (measuredPsd.size() < 2) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "At least two frequency bins are required");
  }

  result.m_processMethod = config.m_method;
  result.m_powerLawIndex = config.m_powerLawIndex;
  result.m_powerLawNormFreq =
      resolvePowerLawNormFreq(freq, config.m_powerLawNormFreq);
  result.m_powerLawMatchFreq = config.m_powerLawMatchFreq;
  result.m_noiseEstimateDomain =
      normalizeNoiseEstimateDomain(config.m_noiseEstimateDomain);
  result.m_noiseEstimateRange =
      normalizeNoiseEstimateRange(config.m_noiseEstimateRange);
  result.m_noiseEstimateLowFreqMaxHz = config.m_noiseEstimateLowFreqMaxHz;
  result.m_noiseEstimateStatistic =
      normalizeNoiseEstimateStatistic(config.m_noiseEstimateStatistic);
  result.m_closedLoopOlEstimateMethod =
      normalizeClosedLoopOlEstimateMethod(config.m_closedLoopOlEstimateMethod);
  result.m_powerLawMatchFallbackWindowHz =
      config.m_powerLawMatchFallbackWindowHz;
  result.m_powerLawCrossoverMode =
      normalizePowerLawCrossoverMode(config.m_powerLawCrossoverMode);
  ...
  result.m_powerLawFitIncludesMatchPoint =
  ...
  if (config.m_dropoutGapFactor <= static_cast<realT>(0) ||
      config.m_dropoutGapFactor >= static_cast<realT>(1)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Dropout gap factor must be between 0 and 1");
  }
  ...
  if (config.m_powerLawBlendBins < 0) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Power-law blend bins must be non-negative");
  }

  if (config.m_dropoutMaxBins < 1) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Dropout max bins must be at least one");
  }

  if (config.m_fitPowerLawIndex &&
      (config.m_powerLawFitMinFreqHz <= static_cast<realT>(0) ||
  ...
  if (result.m_noiseEstimateDomain != "open-loop" &&
      result.m_noiseEstimateDomain != "closed-loop-pre-xfer") {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg,
        "Unknown noise-estimation domain: " + result.m_noiseEstimateDomain);
  }

  if (result.m_noiseEstimateRange != "high-freq" &&
      result.m_noiseEstimateRange != "low-freq") {
    return mx::error_report<mx::verbose::d>(mx::error_t::invalidarg,
                                            "Unknown noise-estimation range: " +
                                                result.m_noiseEstimateRange);
  }

  if (result.m_noiseEstimateStatistic != "percentile" &&
      result.m_noiseEstimateStatistic != "minimum") {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Unknown noise-estimation statistic: " +
                                     result.m_noiseEstimateStatistic);
  }
  ...
  if (result.m_closedLoopOlEstimateMethod != "etf-only" &&
      result.m_closedLoopOlEstimateMethod != "ntf-aware") {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Unknown closed-loop OL estimate method: " +
                                     result.m_closedLoopOlEstimateMethod);
  }

  if (result.m_powerLawCrossoverMode != "manual" &&
      result.m_powerLawCrossoverMode != "auto-smoothed-crossing") {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg,
        "Unknown power-law crossover mode: " + result.m_powerLawCrossoverMode);
  }
  ...
  mx::error_t errc = estimateNoisePsd(
      result.m_noisePsd, result.m_noiseFloor, noiseEstimatePsd, freq, modeIndex,
      result.m_noiseEstimateRange, result.m_noiseEstimateStatistic,
  ...
  if (result.m_noiseEstimateDomain == "closed-loop-pre-xfer") {
    ...
    if (etfPsd->size() != measuredPsd.size()) {
      return mx::error_report<mx::verbose::d>(
          mx::error_t::sizeerr, "ETF PSD must match the measured PSD size");
    }
    ...
    if (result.m_closedLoopOlEstimateMethod == "ntf-aware") {
      ...
      if (ntfPsd->size() != measuredPsd.size()) {
        return mx::error_report<mx::verbose::d>(
            mx::error_t::sizeerr, "NTF PSD must match the measured PSD size");
      }
    }
  ...
  errc = resolvePowerLawCrossoverFrequencies(
      effectiveConfig.m_powerLawMatchFreq,
      effectiveConfig.m_powerLawOnlyAboveFreq, result.m_rawProcessPsd,
      result.m_smoothedProcessPsd, processNoisePsd, freq,
  ...
  if (effectiveConfig.m_method == "legacy") {
    ...
    errc = estimateProcessPsd(result.m_processPsd, result.m_extrapolation,
                              processMeasuredPsd, processNoisePsd, freq,
    ...
    errc = estimateProcessPsdPowerLawOnly(
        result.m_processPsd, result.m_extrapolation,
        result.m_powerLawAnchorIndex, processRepairMask, processMeasuredPsd,
        processNoisePsd, freq, effectiveConfig, &usedPowerLawIndex,
        &fitBinsUsed);
    if (!!errc) {
      return errc;
    }
    ...
    result.m_powerLawIndexFitSucceeded =
        effectiveConfig.m_fitPowerLawIndex && fitBinsUsed > 0;
    result.m_powerLawFitBinsUsed = fitBinsUsed;
  } else if (effectiveConfig.m_method == "moffat-peaks") {
    size_t anchorIndex = 0;
    errc = estimateProcessPsdMoffatPeaks(
        result.m_processPsd, result.m_extrapolation, result.m_peaks,
        processRepairMask, processMeasuredPsd, processNoisePsd, freq,
        effectiveConfig);
    if (!!errc) {
      return errc;
    }
    ...
    realT usedPowerLawIndex = effectiveConfig.m_powerLawIndex;
    size_t fitBinsUsed = 0;
    errc = estimatePowerLawContinuum(
        continuumPsd, result.m_extrapolation, anchorIndex, rawProcessPsd,
        processNoisePsd, freq, effectiveConfig.m_powerLawIndex,
        effectiveConfig.m_powerLawNormFreq, effectiveConfig.m_powerLawMatchFreq,
        effectiveConfig.m_powerLawMatchFallbackWindowHz,
        ...
        effectiveConfig.m_powerLawFitIncludesMatchPoint, &usedPowerLawIndex,
        &fitBinsUsed);
    if (!!errc) {
      return errc;
    }
    ...
    return mx::error_report<mx::verbose::d>(mx::error_t::invalidarg,
                                            "Unknown process method: " +
                                                effectiveConfig.m_method);
  }

  if (effectiveConfig.m_method == "power-law-only" ||
      effectiveConfig.m_method == "moffat-peaks") {
    errc = fillProcessPsdDropouts(result.m_processPsd, freq, processRepairMask,
                                  effectiveConfig.m_dropoutGapFactor,
                                  effectiveConfig.m_dropoutMaxBins);
    if (!!errc) {
      return errc;
    ...
    if (effectiveConfig.m_powerLawMatchFreq > static_cast<realT>(0)) {
      ...
      errc = estimatePowerLawContinuum(
          continuumPsd, result.m_extrapolation, rematchAnchorIndex,
          rawProcessPsd, processNoisePsd, freq, result.m_powerLawIndex,
          effectiveConfig.m_powerLawNormFreq, static_cast<realT>(0),
          effectiveConfig.m_powerLawMatchFallbackWindowHz, false);
      if (!!errc) {
        return errc;
      }

      errc = matchPowerLawAtFreq(
          result.m_extrapolation, result.m_processPsd, freq,
          result.m_powerLawIndex, effectiveConfig.m_powerLawNormFreq,
          effectiveConfig.m_powerLawMatchFreq,
          effectiveConfig.m_powerLawMatchFallbackWindowHz);
      if (!!errc) {
        return errc;
      ...
      errc = buildPowerLawContinuum(continuumPsd, result.m_extrapolation, freq,
      ...
      if (effectiveConfig.m_method == "power-law-only") {
        errc = buildPowerLawOnlyProcessFromContinuum(
            result.m_processPsd, processRepairMask, rawProcessPsd,
            processNoisePsd, continuumPsd, freq, result.m_powerLawAnchorIndex,
            effectiveConfig);
      } else {
        errc = buildMoffatProcessFromContinuum(
            result.m_processPsd, result.m_peaks, processRepairMask,
            rawProcessPsd, processNoisePsd, continuumPsd, freq,
            result.m_powerLawAnchorIndex, effectiveConfig);
      }
      if (!!errc) {
        return errc;
      }

      errc = fillProcessPsdDropouts(
          result.m_processPsd, freq, processRepairMask,
          effectiveConfig.m_dropoutGapFactor, effectiveConfig.m_dropoutMaxBins);
      if (!!errc) {
        return errc;
      }
    }
  ...
  errc = applyLpContinuum(result.m_lpProcessPsd, result.m_processPsd, freq,
                          lpContinuumFreq, lpContinuumWidthHz);
  if (!!errc) {
    return errc;
  }
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::estimateNoisePsd(
    std::vector<realT> &noisePsd, realT &noiseFloor,
    const std::vector<realT> &measuredPsd, const std::vector<realT> &freq,
    size_t modeIndex, std::string noiseEstimateRange,
    std::string noiseEstimateStatistic, realT noiseEstimateLowFreqMaxHz) {
  ...
  noiseEstimateStatistic =
      normalizeNoiseEstimateStatistic(noiseEstimateStatistic);
  ...
  if (noiseEstimateRange == "low-freq") {
    ...
    if (noiseEstimateLowFreqMaxHz > static_cast<realT>(0)) {
      size_t cappedF1 = f0;
      while (cappedF1 < measuredPsd.size() &&
             freq[cappedF1] <= noiseEstimateLowFreqMaxHz) {
        ++cappedF1;
      }
  ...
  if (f1 <= f0) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "PSD is too short for noise fitting");
  }
...
template <typename realT>
std::string
modalPsdProcessor<realT>::normalizeNoiseEstimateDomain(std::string domain) {
...
template <typename realT>
std::string
modalPsdProcessor<realT>::normalizeNoiseEstimateRange(std::string range) {
...
template <typename realT>
std::string modalPsdProcessor<realT>::normalizeNoiseEstimateStatistic(
...
template <typename realT>
std::string modalPsdProcessor<realT>::normalizeClosedLoopOlEstimateMethod(
    std::string method) {
...
template <typename realT>
std::string
modalPsdProcessor<realT>::normalizePowerLawCrossoverMode(std::string mode) {
...
template <typename realT>
size_t modalPsdProcessor<realT>::firstPositiveFreqIndex(
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::buildSmoothedProcessPsd(
    std::vector<realT> &smoothedProcessPsd,
    const std::vector<realT> &rawProcessPsd, const std::vector<realT> &freq,
    realT smoothWidthHz) {
  if (rawProcessPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Smoothed disturbance PSD inputs must have "
  ...
  if (firstPositive >= freq.size() ||
      freq[firstPositive] <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg,
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::findAutoPowerLawCrossoverFreq(
    realT &crossoverFreq, const std::vector<realT> &smoothedProcessPsd,
    const std::vector<realT> &noisePsd, const std::vector<realT> &freq) {
  if (smoothedProcessPsd.size() != noisePsd.size() ||
      smoothedProcessPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Automatic power-law crossover inputs must have "
                              "matching PSD and frequency sizes");
  }
  ...
  for (size_t n = firstPositive; n + 1 < smoothedProcessPsd.size(); ++n) {
    ...
    if (d0 >= static_cast<realT>(0) && d1 <= static_cast<realT>(0)) {
      ...
      alpha = std::max(static_cast<realT>(0),
                       std::min(static_cast<realT>(1), alpha));
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::resolvePowerLawCrossoverFrequencies(
    realT &powerLawMatchFreq, realT &powerLawOnlyAboveFreq,
  ...
  mx::error_t errc = findAutoPowerLawCrossoverFreq(
      crossoverFreq, smoothedProcessPsd, noisePsd, freq);
  if (!!errc) {
    return errc;
  }
...
template <typename realT>
realT modalPsdProcessor<realT>::resolvePowerLawNormFreq(
    const std::vector<realT> &freq, realT requestedNormFreq) {
...
realT modalPsdProcessor<realT>::powerLawContinuum(
    realT extrapolation, const std::vector<realT> &freq, size_t index,
    realT powerLawIndex, realT powerLawNormFreq) {
  const realT refFreq = resolvePowerLawNormFreq(freq, powerLawNormFreq);
  const realT useFreq =
      freq[index] > static_cast<realT>(0) ? freq[index] : refFreq;

  return extrapolation * pow(refFreq / useFreq, powerLawIndex);
}

template <typename realT>
realT modalPsdProcessor<realT>::interpolatePsdAtFreq(
    const std::vector<realT> &psd, const std::vector<realT> &freq,
    realT targetFreq) {
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::matchPowerLawAtFreq(
    realT &extrapolation, const std::vector<realT> &rawProcessPsd,
    const std::vector<realT> &freq, realT powerLawIndex, realT powerLawNormFreq,
    realT powerLawMatchFreq, realT powerLawMatchFallbackWindowHz) {
  ...
  if (firstPositive >= freq.size() ||
      freq[firstPositive] <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Frequency grid must contain positive bins");
  }

  if (powerLawMatchFreq < freq[firstPositive] ||
      powerLawMatchFreq > freq.back()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg,
  ...
  const realT localMinFreq = std::max(
      freq[firstPositive], powerLawMatchFreq - powerLawMatchFallbackWindowHz);
  const realT localMaxFreq =
      std::min(freq.back(), powerLawMatchFreq + powerLawMatchFallbackWindowHz);
  size_t localStart =
      std::lower_bound(freq.begin(), freq.end(), localMinFreq) - freq.begin();
  size_t localEnd =
      std::upper_bound(freq.begin(), freq.end(), localMaxFreq) - freq.begin();
  ...
  if (!localPositivePsd.empty()) {
    ...
    std::nth_element(localPositivePsd.begin(), localPositivePsd.begin() + mid,
                     localPositivePsd.end());
    realT localMedian = localPositivePsd[mid];
    if (localPositivePsd.size() % 2 == 0) {
      auto lowerIt = std::max_element(localPositivePsd.begin(),
                                      localPositivePsd.begin() + mid);
  ...
  extrapolation = std::max(matchPsd, tiny) *
                  pow(powerLawMatchFreq / refFreq, powerLawIndex);
  return mx::error_t::noerror;
}
...
realT modalPsdProcessor<realT>::moffatAlphaFromFwhm(realT fwhm, realT beta) {
  return fwhm / (static_cast<realT>(2) *
                 sqrt(pow(static_cast<realT>(2), static_cast<realT>(1) / beta) -
                      static_cast<realT>(1)));
}

template <typename realT>
realT modalPsdProcessor<realT>::moffatValueFromFwhm(realT radius,
                                                    realT peakHeight,
                                                    realT fwhm, realT beta) {
  const realT alpha = moffatAlphaFromFwhm(fwhm, beta);
  return mx::math::func::moffat<realT>(radius, static_cast<realT>(0),
                                       peakHeight, static_cast<realT>(0), alpha,
                                       beta);
}
...
realT modalPsdProcessor<realT>::fitMoffatBetaToBoundary(
  ...
  if (peakHeight <= static_cast<realT>(0) || fwhm <= static_cast<realT>(0) ||
      radius <= static_cast<realT>(0) || target <= static_cast<realT>(0)) {
    return betaFloor;
  }
  ...
  realT high =
      std::max(static_cast<realT>(2) * low, low + static_cast<realT>(0.5));
  while (high < static_cast<realT>(128) &&
         moffatValueFromFwhm(radius, peakHeight, fwhm, high) > target) {
...
template <typename realT>
realT modalPsdProcessor<realT>::interpolateCrossing(realT x0, realT y0,
                                                    realT x1, realT y1,
                                                    realT target) {
  if (y1 == y0) {
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::fitPowerLawIndexFromBinnedMedians(
    realT &powerLawIndex, size_t &nBinsUsed,
    const std::vector<realT> &rawProcessPsd, const std::vector<realT> &freq,
    realT fitMinFreqHz, realT fitMaxFreqHz, realT fitBinWidthHz,
    realT includeMatchFreqHz) {
  ...
  size_t idx =
      std::lower_bound(freq.begin(), freq.end(), fitMinFreqHz) - freq.begin();
  for (realT binStart = fitMinFreqHz; binStart < fitMaxFreqHz;
       binStart += fitBinWidthHz) {
  ...
  if (includeMatchFreqHz > static_cast<realT>(0) &&
      (includeMatchFreqHz < fitMinFreqHz ||
       includeMatchFreqHz >= fitMaxFreqHz)) {
    realT matchPsd =
        interpolatePsdAtFreq(rawProcessPsd, freq, includeMatchFreqHz);
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::estimatePowerLawContinuum(
    std::vector<realT> &continuumPsd, realT &extrapolation, size_t &anchorIndex,
    const std::vector<realT> &rawProcessPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &freq, realT powerLawIndex, realT powerLawNormFreq,
    realT powerLawMatchFreq, realT powerLawMatchFallbackWindowHz,
    bool fitPowerLawIndex, realT powerLawFitMinFreqHz,
    realT powerLawFitMaxFreqHz, realT powerLawFitBinWidthHz,
    bool powerLawFitIncludesMatchPoint, realT *usedPowerLawIndex,
    size_t *fitBinsUsed) {
  if (rawProcessPsd.size() != noisePsd.size() ||
      rawProcessPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Raw disturbance PSD, noise PSD, and frequency "
  ...
  if (powerLawIndex < static_cast<realT>(0) && !fitPowerLawIndex) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Power-law index must be non-negative");
  }
  ...
  if (fitPowerLawIndex) {
    mx::error_t errc = fitPowerLawIndexFromBinnedMedians(
        localPowerLawIndex, localFitBinsUsed, rawProcessPsd, freq,
        powerLawFitMinFreqHz, powerLawFitMaxFreqHz, powerLawFitBinWidthHz,
  ...
  size_t fMax = static_cast<size_t>(static_cast<realT>(0.05) *
                                    static_cast<realT>(freq.size()));
  if (fMax < 2) {
  ...
  for (size_t f = 1; f < fMax; ++f) {
    ...
    extrapolation +=
        log10(rawProcessPsd[f] * pow(freq[f] / refFreq, localPowerLawIndex));
    anchorIndex = f;
    ++nExtrap;
  }

  if (nExtrap > 0) {
    extrapolation = pow(static_cast<realT>(10),
                        extrapolation / static_cast<realT>(nExtrap));
  } else {
    const realT useFreq =
        freq[refIndex] > static_cast<realT>(0) ? freq[refIndex] : refFreq;
    extrapolation = std::max(
        rawProcessPsd[refIndex] *
            static_cast<realT>(pow(useFreq / refFreq, localPowerLawIndex)),
        tiny);
  }

  mx::error_t errc = matchPowerLawAtFreq(
      extrapolation, rawProcessPsd, freq, localPowerLawIndex, refFreq,
      powerLawMatchFreq, powerLawMatchFallbackWindowHz);
  if (!!errc) {
    return errc;
  }

  errc = buildPowerLawContinuum(continuumPsd, extrapolation, freq,
                                localPowerLawIndex, refFreq);
  if (!!errc) {
    return errc;
  }
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::buildPowerLawContinuum(
    std::vector<realT> &continuumPsd, realT extrapolation,
    const std::vector<realT> &freq, realT powerLawIndex,
  ...
  for (size_t n = 0; n < freq.size(); ++n) {
    continuumPsd[n] =
        std::max(powerLawContinuum(extrapolation, freq, n, powerLawIndex,
                                   powerLawNormFreq),
                 tiny);
  }
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::blendContinuumAtAnchor(
    std::vector<realT> &processPsd, const std::vector<realT> &rawProcessPsd,
    const std::vector<realT> &continuumPsd, size_t anchorIndex, int blendBins) {
  ...
  if (rawProcessPsd.empty()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "At least one frequency bin is required");
  }
  ...
  size_t blendEnd = std::min(anchorIndex + static_cast<size_t>(blendBins),
                             rawProcessPsd.size() - 1);
  ...
  for (size_t n = 0; n < processPsd.size(); ++n) {
    ...
    realT w = static_cast<realT>(n - anchorIndex) /
              static_cast<realT>(blendEnd - anchorIndex);
    realT logBlend =
        (static_cast<realT>(1) - w) * log10(std::max(rawProcessPsd[n], tiny)) +
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::identifyMoffatPeaks(
    std::vector<identifiedPeak1D> &peaks,
    const std::vector<realT> &rawProcessPsd,
    const std::vector<realT> &continuumPsd, const std::vector<realT> &freq,
    realT peakDetectWidthHz, realT peakDetectFactor,
    realT peakDetectBroadFactor, realT peakDetectMinWidthLog) {
  if (rawProcessPsd.size() != continuumPsd.size() ||
      rawProcessPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Raw disturbance PSD, continuum PSD, and "
                              "frequency grid must have the same size");
  }

  if (rawProcessPsd.size() < 3) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "At least three frequency bins are required");
  }

  if (peakDetectWidthHz <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Peak-detection width must be positive");
  }

  if (peakDetectFactor <= static_cast<realT>(1)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Peak-detection factor must exceed unity");
  }
  ...
  if (df <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Frequency spacing must be positive");
  }
  ...
  for (size_t p = 0; p < peaks.size(); ++p) {
    ...
    peak.m_peakHeight =
        rawProcessPsd[peak.m_peakIndex] - continuumPsd[peak.m_peakIndex];
    if (peak.m_peakHeight <= static_cast<realT>(0)) {
      continue;
    }

    const realT halfLevel = continuumPsd[peak.m_peakIndex] +
                            static_cast<realT>(0.5) * peak.m_peakHeight;
    const realT defaultFwhm =
        std::max(df, static_cast<realT>(peak.m_end - peak.m_start + 1) * df);
    ...
    for (size_t n = peak.m_peakIndex; n > 0; --n) {
      if (rawProcessPsd[n - 1] <= halfLevel) {
        leftCross = interpolateCrossing(freq[n - 1], rawProcessPsd[n - 1],
                                        freq[n], rawProcessPsd[n], halfLevel);
        break;
      }
    }

    realT rightCross =
        peak.m_centerFreq + static_cast<realT>(0.5) * defaultFwhm;
    for (size_t n = peak.m_peakIndex; n + 1 < rawProcessPsd.size(); ++n) {
      if (rawProcessPsd[n + 1] <= halfLevel) {
        rightCross = interpolateCrossing(freq[n], rawProcessPsd[n], freq[n + 1],
                                         rawProcessPsd[n + 1], halfLevel);
        break;
      }
    }
    ...
    realT lowerFreq =
        std::max(peak.m_centerFreq - static_cast<realT>(0.5) * peak.m_fwhm,
                 minPositiveFreq);
    realT upperFreq = std::max(
        peak.m_centerFreq + static_cast<realT>(0.5) * peak.m_fwhm, lowerFreq);
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::addClippedMoffatPeakExcess(
    std::vector<realT> &peakModel, const identifiedPeak1D &peak,
    const std::vector<realT> &sourcePsd, const std::vector<realT> &continuumPsd,
    const std::vector<realT> &freq, realT peakMoffatBeta) {
  if (peakModel.size() != sourcePsd.size() ||
      peakModel.size() != continuumPsd.size() ||
      peakModel.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Peak model, source PSD, continuum PSD, and "
                              "frequency grid must have the same size");
  }

  if (peak.m_peakIndex >= peakModel.size() ||
      peak.m_fwhm <= static_cast<realT>(0)) {
    return mx::error_t::noerror;
  }
  ...
  if (peak.m_start > 0 && peak.m_start <= peak.m_peakIndex) {
    ...
    peakBeta =
        std::max(peakBeta,
                 fitMoffatBetaToBoundary(peak.m_peakHeight, peak.m_fwhm, radius,
                                         continuumPsd[edge], peakMoffatBeta));
  }
  ...
  realT peakAtCenter = mx::math::func::moffat<realT>(
      peak.m_centerFreq, static_cast<realT>(0), peak.m_peakHeight,
      peak.m_centerFreq, alpha, peakBeta);
  ...
  while (right + 1 < peakModel.size() &&
         sourcePsd[right + 1] > continuumPsd[right + 1]) {
    ++right;
  }

  for (size_t n = left; n <= right; ++n) {
    realT peakValue = mx::math::func::moffat<realT>(
        freq[n], static_cast<realT>(0), peak.m_peakHeight, peak.m_centerFreq,
        alpha, peakBeta);
    realT measuredExcess =
        std::max(sourcePsd[n] - continuumPsd[n], static_cast<realT>(0));
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::identifyMoffatPeaksMultiPass(
    std::vector<identifiedPeak1D> &peaks,
    const std::vector<realT> &rawProcessPsd,
    const std::vector<realT> &continuumPsd, const std::vector<realT> &freq,
    realT peakDetectWidthHz, realT peakDetectFactor,
    realT peakDetectBroadFactor, realT peakDetectMinWidthLog,
    int peakDetectPasses, realT peakMoffatBeta) {
  if (peakDetectPasses < 1) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Peak-detection passes must be positive");
  }
  ...
  for (size_t n = 0; n < workingExcess.size(); ++n) {
    workingExcess[n] =
        std::max(rawProcessPsd[n] - continuumPsd[n], static_cast<realT>(0));
  }
  ...
  for (int pass = 0; pass < peakDetectPasses; ++pass) {
    ...
    mx::error_t errc = identifyMoffatPeaks(
        passPeaks, workingPsd, continuumPsd, freq, peakDetectWidthHz,
        peakDetectFactor, peakDetectBroadFactor, peakDetectMinWidthLog);
    if (!!errc) {
      return errc;
    }
    ...
    for (size_t p = 0; p < passPeaks.size(); ++p) {
      errc = addClippedMoffatPeakExcess(passModel, passPeaks[p], workingPsd,
                                        continuumPsd, freq, peakMoffatBeta);
      if (!!errc) {
        return errc;
      }
    }
    ...
    for (size_t n = 0; n < workingExcess.size(); ++n) {
      realT newExcess =
          std::max(workingExcess[n] - passModel[n], static_cast<realT>(0));
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::buildMoffatProcessFromContinuum(
    std::vector<realT> &processPsd, std::vector<identifiedPeak1D> &peaks,
    std::vector<unsigned char> &repairMask,
    const std::vector<realT> &rawProcessPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &continuumPsd, const std::vector<realT> &freq,
    size_t anchorIndex, const processModelConfig &config) {
  if (rawProcessPsd.size() != noisePsd.size() ||
      rawProcessPsd.size() != continuumPsd.size() ||
      rawProcessPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr, "Raw disturbance PSD, noise PSD, continuum PSD, "
                              "and frequency grid must have the same size");
  }

  const realT tiny = std::numeric_limits<realT>::min();

  mx::error_t errc = identifyMoffatPeaksMultiPass(
      peaks, rawProcessPsd, continuumPsd, freq, config.m_peakDetectWidthHz,
      config.m_peakDetectFactor, config.m_peakDetectBroadFactor,
      config.m_peakDetectMinWidthLog, config.m_peakDetectPasses,
  ...
  for (size_t p = 0; p < peaks.size(); ++p) {
    errc =
        addClippedMoffatPeakExcess(peakModel, peaks[p], rawProcessPsd,
                                   continuumPsd, freq, config.m_peakMoffatBeta);
    if (!!errc) {
      return errc;
    }
  }
  ...
  for (size_t n = 0; n < highFreqModel.size(); ++n) {
    if (config.m_powerLawOnlyAboveFreq > static_cast<realT>(0) &&
        freq[n] >= config.m_powerLawOnlyAboveFreq) {
    ...
    realT measuredExcess =
        std::max(rawProcessPsd[n] - continuumPsd[n], static_cast<realT>(0));
    highFreqModel[n] = std::max(
        continuumPsd[n] + std::min(peakModel[n], measuredExcess), tiny);
  }

  std::vector<realT> extrapolatedPsd;
  errc = blendContinuumAtAnchor(extrapolatedPsd, rawProcessPsd, highFreqModel,
                                anchorIndex, config.m_powerLawBlendBins);
  if (!!errc) {
    return errc;
  }
  ...
  size_t blendEnd =
      std::min(anchorIndex + static_cast<size_t>(config.m_powerLawBlendBins),
               rawProcessPsd.size() - 1);
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::buildPowerLawOnlyProcessFromContinuum(
    std::vector<realT> &processPsd, std::vector<unsigned char> &repairMask,
    const std::vector<realT> &rawProcessPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &continuumPsd, const std::vector<realT> &freq,
    size_t anchorIndex, const processModelConfig &config) {
  if (rawProcessPsd.size() != noisePsd.size() ||
  ...
  std::vector<realT> extrapolatedPsd;
  mx::error_t errc =
      blendContinuumAtAnchor(extrapolatedPsd, rawProcessPsd, continuumPsd,
                             anchorIndex, config.m_powerLawBlendBins);
  if (!!errc) {
    return errc;
  }
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::estimateProcessPsdPowerLawOnly(
    std::vector<realT> &processPsd, realT &extrapolation, size_t &anchorIndex,
    std::vector<unsigned char> &repairMask,
    const std::vector<realT> &measuredPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &freq, const processModelConfig &config,
    realT *usedPowerLawIndex, size_t *fitBinsUsed) {
  if (measuredPsd.size() != noisePsd.size() ||
      measuredPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr,
  ...
  std::vector<realT> continuumPsd;
  mx::error_t errc = estimatePowerLawContinuum(
      continuumPsd, extrapolation, anchorIndex, rawProcessPsd, noisePsd, freq,
      config.m_powerLawIndex, config.m_powerLawNormFreq,
      config.m_powerLawMatchFreq, config.m_powerLawMatchFallbackWindowHz,
      config.m_fitPowerLawIndex, config.m_powerLawFitMinFreqHz,
      config.m_powerLawFitMaxFreqHz, config.m_powerLawFitBinWidthHz,
      config.m_powerLawFitIncludesMatchPoint, usedPowerLawIndex, fitBinsUsed);
  if (!!errc) {
    return errc;
  }

  return buildPowerLawOnlyProcessFromContinuum(
      processPsd, repairMask, rawProcessPsd, noisePsd, continuumPsd, freq,
      anchorIndex, config);
}

template <typename realT>
mx::error_t modalPsdProcessor<realT>::estimateProcessPsdMoffatPeaks(
    std::vector<realT> &processPsd, realT &extrapolation,
    std::vector<identifiedPeak1D> &peaks,
    std::vector<unsigned char> &repairMask,
    const std::vector<realT> &measuredPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &freq, const processModelConfig &config) {
  ...
  if (config.m_peakMoffatBeta <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Peak Moffat beta must be positive");
  }
  ...
  return buildMoffatProcessFromContinuum(processPsd, peaks, repairMask,
                                         rawProcessPsd, noisePsd, continuumPsd,
                                         freq, anchorIndex, config);
}

template <typename realT>
mx::error_t modalPsdProcessor<realT>::estimateProcessPsd(
    std::vector<realT> &processPsd, realT &extrapolation,
    const std::vector<realT> &measuredPsd, const std::vector<realT> &noisePsd,
    const std::vector<realT> &freq, realT powerLawNormFreq,
    realT powerLawMatchFreq, realT powerLawMatchFallbackWindowHz) {
  if (measuredPsd.size() != noisePsd.size() ||
      measuredPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
  ...
  for (size_t f = 1; f < fMax; ++f) {
    ...
    extrapolation +=
        log10(processPsd[f] * pow(freq[f] / refFreq, c_defaultPowerLawIndex));
    ++nExtrap;
  }
  ...
  if (nExtrap > 0) {
    ...
    extrapolation = std::max(
        processPsd[refIndex] *
            static_cast<realT>(pow(useFreq / refFreq, c_defaultPowerLawIndex)),
        tiny);
  }

  mx::error_t errc = matchPowerLawAtFreq(
      extrapolation, processPsd, freq, c_defaultPowerLawIndex, refFreq,
      powerLawMatchFreq, powerLawMatchFallbackWindowHz);
  if (!!errc) {
    return errc;
  }
  ...
  for (size_t f = 0; f < freq.size(); ++f) {
    if (processPsd[f] < static_cast<realT>(0)) {
      ...
      toSmooth[f] =
          extrapolation * pow(refFreq / useFreq, c_defaultPowerLawIndex);
    } else {
      toSmooth[f] = processPsd[f];
    }

    toSmooth[f] = std::max(toSmooth[f], tiny);
    smoothWidths[f] = f < 2 ? 2
                            : 2 + static_cast<int>(static_cast<realT>(f) /
                                                   static_cast<realT>(10));
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::fillProcessPsdDropouts(
    std::vector<realT> &processPsd, const std::vector<realT> &freq,
    const std::vector<unsigned char> &repairMask, realT gapFactor,
    size_t maxGapBins) {
  ...
  if (gapFactor <= static_cast<realT>(0) ||
      gapFactor >= static_cast<realT>(1)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "Dropout gap factor must be between 0 and 1");
  }
...
template <typename realT>
mx::error_t modalPsdProcessor<realT>::applyLpContinuum(
    std::vector<realT> &lpProcessPsd, const std::vector<realT> &processPsd,
    const std::vector<realT> &freq, realT cutoffFreq, realT continuumWidthHz) {
  if (processPsd.size() != freq.size()) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::sizeerr,
  ...
  if (continuumWidthHz <= static_cast<realT>(0)) {
    return mx::error_report<mx::verbose::d>(
        mx::error_t::invalidarg, "LP continuum width must be positive");
  }
  ...
  for (size_t n = cutIndex; n < lpProcessPsd.size(); ++n) {
    lpProcessPsd[n] =
        std::max(static_cast<realT>(pow(static_cast<realT>(10),
                                        logContinuumTail[n - cutIndex])),
                 tiny);
  }
