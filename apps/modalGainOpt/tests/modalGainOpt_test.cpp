...
public:
  void readConfigFile(const std::string &path) { config.readConfig(path); }

  int shutdownState() const { return m_shutdown; }

  float gainGain() const { return m_gainGain; }

  int extrapMethod() const { return m_extrapOL; }

  int extrapNoiseEstimateDomain() const { return m_extrapNoiseEstimateDomain; }

  int extrapNoiseEstimateRange() const { return m_extrapNoiseEstimateRange; }
  ...
  void setExtrapMethodForTest(int method) { m_extrapOL = method; }
  ...
  void setExtrapNoiseEstimateDomainForTest(int domain) {
    ...
    m_extrapConfig.m_noiseEstimateDomain =
        extrapNoiseEstimateDomainName(domain);
  }
  ...
  void setExtrapNoiseEstimateStatisticForTest(int statistic) {
    ...
    m_extrapConfig.m_noiseEstimateStatistic =
        extrapNoiseEstimateStatisticName(statistic);
  }
  ...
  void setExtrapClosedLoopOlEstimateMethodForTest(int method) {
    ...
    m_extrapConfig.m_closedLoopOlEstimateMethod =
        extrapClosedLoopOlEstimateMethodName(method);
  }
  ...
  void setExtrapPowerLawCrossoverModeForTest(int mode) {
    ...
    m_extrapConfig.m_powerLawCrossoverMode =
        extrapPowerLawCrossoverModeName(mode);
  }
  ...
  int modesOn() const { return m_modesOn; }

  int sinceChange() const { return m_sinceChange; }

  bool goptUpdated() const { return m_goptUpdated; }

  bool pcgoptUpdated() const { return m_pcgoptUpdated; }

  bool freqUpdated() const { return m_freqUpdated; }

  float fps() const { return m_fps; }

  const std::vector<float> &freq() const { return m_freq; }

  void setPcOnForTest(bool pcOn) { m_pcOn = pcOn; }

  void setLoopForTest(bool loop) { m_loop = loop; }

  void setModesOnForTest(int modesOn) { m_modesOn = modesOn; }

  void setSinceChangeForTest(int sinceChange) { m_sinceChange = sinceChange; }

  void setGoptUpdatedForTest(bool goptUpdated) { m_goptUpdated = goptUpdated; }

  void setPcgoptUpdatedForTest(bool pcgoptUpdated) {
    m_pcgoptUpdated = pcgoptUpdated;
  }

  void setFreqUpdatedForTest(bool freqUpdated) { m_freqUpdated = freqUpdated; }

  void setFpsForTest(float fps) { m_fps = fps; }

  void setFreqForTest(const std::vector<float> &freq) { m_freq = freq; }

  void configurePublishedGainState(
      const std::vector<float> &gainCalFacts, const std::vector<float> &gainSI,
      const std::vector<float> &gainMaxSI, const std::vector<float> &gainLP,
      const std::vector<float> &gainMaxLP, const std::vector<float> &gainCals,
      const std::vector<float> &modeVarOL, const std::vector<float> &modeVarSI,
      const std::vector<float> &modeVarLP, float opticalGain) {
  ...
  void writePublishedGainArraysForTest(float *currentData, float *siData,
                                       float *maxSiData, float *lpData,
                                       float *maxLpData, float *modeVarData) {
    writePublishedGainArrays(currentData, siData, maxSiData, lpData, maxLpData,
                             modeVarData);
  }

  void configurePublishedPredictorState(
      const std::vector<float> &gainCalFacts, const std::vector<float> &gainLP,
      const std::vector<float> &gainCals, const std::vector<uint32_t> &Na,
      const std::vector<uint32_t> &Nb,
      const std::vector<std::vector<float>> &aCoeff,
      const std::vector<std::vector<float>> &bCoeff, float opticalGain,
      float gainGain) {
    m_gainCalFacts = gainCalFacts;
    m_optGainLP = gainLP;
    m_gainCals = gainCals;
  ...
  void writePublishedPredictorArraysForTest(float *pcGainData,
                                            float *aCoeffData, uint32_t aWidth,
                                            float *bCoeffData, uint32_t bWidth,
                                            bool blend) {
    writePublishedPredictorArrays(pcGainData, aCoeffData, aWidth, bCoeffData,
                                  bWidth, blend);
  }

  int countEnabledGainFactorsForTest(
      const std::vector<float> &gainFacts) const {
    return countEnabledGainFactors(gainFacts);
  }

  void updateAppliedModeCountForTest(const std::vector<float> &gainFacts,
                                     bool predictorPath) {
  ...
  bool applyGainFactorUpdateForTest(std::vector<float> &gainFacts,
                                    ...
                                    bool predictorPath) {
    return applyGainFactorUpdate(gainFacts, incoming.data(), incoming.size(),
                                 predictorPath);
  }
  ...
  bool applyMultiplierUpdateForTest(std::vector<float> &multFacts,
                                    const std::vector<float> &incoming,
                                    bool predictorPath) {
    return applyMultiplierUpdate(multFacts, incoming.data(), incoming.size(),
                                 predictorPath);
  }
  ...
  size_t goptCurrentSize() const { return m_goptCurrent.size(); }
  ...
  void initExtrapSelectionPropertiesForTest() {
    ...
    createStandardIndiSelectionSw(
        ...
        {extrapNoiseEstimateDomainLabel(c_extrapNoiseEstimateOpenLoop),
         extrapNoiseEstimateDomainLabel(
             c_extrapNoiseEstimateClosedLoopPreXfer)},
        "Noise Estimate Domain", "Extrapolation");
    ...
    createStandardIndiSelectionSw(
        ...
        "Noise Estimate Range", "Extrapolation");
    ...
    createStandardIndiSelectionSw(
        ...
        "Noise Estimate Statistic", "Extrapolation");

    createStandardIndiSelectionSw(m_indiP_extrapClosedLoopOlEstimateMethod,
    ...
    createStandardIndiSelectionSw(
        ...
        {extrapPowerLawCrossoverModeLabel(c_extrapPowerLawCrossoverManual),
         extrapPowerLawCrossoverModeLabel(
             c_extrapPowerLawCrossoverAutoSmoothedCrossing)},
        "Power-Law Crossover Mode", "Extrapolation");
  }
 ...
 * \ingroup modalGainOpt_unit_test
 */
TEST_CASE("modalGainOpt placeholder harness instantiates the app",
          "[modalGainOpt]") {
  // clang-format off
  ...
  SECTION("OL process-method helpers map consistently") {
    ...
    REQUIRE(olProcessMethodFromElement("power_law_only") ==
            c_olProcessPowerLawOnly);
    REQUIRE(olProcessMethodFromElement("moffat_peaks") ==
    ...
    REQUIRE(olProcessMethodFromName("power_law_only") ==
            c_olProcessPowerLawOnly);
    REQUIRE(olProcessMethodFromName("power-law-only") ==
            c_olProcessPowerLawOnly);
    ...
    REQUIRE(extrapNoiseEstimateDomainName(c_extrapNoiseEstimateOpenLoop) ==
            "open-loop");
    REQUIRE(
        extrapNoiseEstimateDomainName(c_extrapNoiseEstimateClosedLoopPreXfer) ==
        "closed-loop-pre-xfer");

    REQUIRE(extrapNoiseEstimateDomainFromElement("open_loop") ==
            c_extrapNoiseEstimateOpenLoop);
    REQUIRE(extrapNoiseEstimateDomainFromElement("closed_loop_pre_xfer") ==
            c_extrapNoiseEstimateClosedLoopPreXfer);

    REQUIRE(extrapNoiseEstimateDomainFromName("open_loop") ==
            c_extrapNoiseEstimateOpenLoop);
    REQUIRE(extrapNoiseEstimateDomainFromName("open-loop") ==
            c_extrapNoiseEstimateOpenLoop);
    ...
    REQUIRE(extrapNoiseEstimateRangeName(c_extrapNoiseEstimateHighFreq) ==
            "high-freq");
    REQUIRE(extrapNoiseEstimateRangeName(c_extrapNoiseEstimateLowFreq) ==
            "low-freq");
    REQUIRE(extrapNoiseEstimateRangeFromElement("high_freq") ==
            c_extrapNoiseEstimateHighFreq);
    REQUIRE(extrapNoiseEstimateRangeFromElement("low_freq") ==
            c_extrapNoiseEstimateLowFreq);
    REQUIRE(extrapNoiseEstimateRangeFromName("high_freq") ==
            c_extrapNoiseEstimateHighFreq);
    REQUIRE(extrapNoiseEstimateRangeFromName("high-freq") ==
            c_extrapNoiseEstimateHighFreq);
    REQUIRE(extrapNoiseEstimateRangeFromName("low_freq") ==
            c_extrapNoiseEstimateLowFreq);
    REQUIRE(extrapNoiseEstimateRangeFromName("low-freq") ==
            c_extrapNoiseEstimateLowFreq);

    REQUIRE(extrapNoiseEstimateStatisticName(c_extrapNoiseEstimatePercentile) ==
            "percentile");
    REQUIRE(extrapNoiseEstimateStatisticName(c_extrapNoiseEstimateMinimum) ==
            "minimum");
    REQUIRE(extrapNoiseEstimateStatisticFromElement("percentile") ==
            c_extrapNoiseEstimatePercentile);
    REQUIRE(extrapNoiseEstimateStatisticFromElement("minimum") ==
            c_extrapNoiseEstimateMinimum);
    REQUIRE(extrapNoiseEstimateStatisticFromName("percentile") ==
            c_extrapNoiseEstimatePercentile);
    REQUIRE(extrapNoiseEstimateStatisticFromName("minimum") ==
            c_extrapNoiseEstimateMinimum);

    REQUIRE(extrapClosedLoopOlEstimateMethodName(
                c_extrapClosedLoopOlEstimateEtfOnly) == "etf-only");
    REQUIRE(extrapClosedLoopOlEstimateMethodName(
                c_extrapClosedLoopOlEstimateNtfAware) == "ntf-aware");
    REQUIRE(extrapClosedLoopOlEstimateMethodFromElement("etf_only") ==
            c_extrapClosedLoopOlEstimateEtfOnly);
    REQUIRE(extrapClosedLoopOlEstimateMethodFromElement("ntf_aware") ==
            c_extrapClosedLoopOlEstimateNtfAware);
    REQUIRE(extrapClosedLoopOlEstimateMethodFromName("etf_only") ==
            c_extrapClosedLoopOlEstimateEtfOnly);
    REQUIRE(extrapClosedLoopOlEstimateMethodFromName("etf-only") ==
            c_extrapClosedLoopOlEstimateEtfOnly);
    REQUIRE(extrapClosedLoopOlEstimateMethodFromName("ntf_aware") ==
            c_extrapClosedLoopOlEstimateNtfAware);
    REQUIRE(extrapClosedLoopOlEstimateMethodFromName("ntf-aware") ==
            c_extrapClosedLoopOlEstimateNtfAware);
  }
}
...
TEST_CASE("modalGainOpt configuration loads PSD-processing settings without "
  ...
  mx::app::writeConfigFile(
      ...
      {"loop",          "loop",          "loop",          "loop",
       "loop",          "extrapolation", "extrapolation", "extrapolation",
  ...
  REQUIRE(app.extrapNoiseEstimateDomain() ==
          c_extrapNoiseEstimateClosedLoopPreXfer);
  ...
  REQUIRE(app.extrapClosedLoopOlEstimateMethod() ==
          c_extrapClosedLoopOlEstimateNtfAware);
  ...
  REQUIRE(app.extrapConfig().m_powerLawCrossoverMode ==
...
TEST_CASE("modalGainOpt restores current selection when extrapolation switches "
  ...
  SECTION("extrapolation method is restored") {
    ...
    ip.add(pcf::IndiElement(olProcessMethodElement(c_olProcessNone),
                            pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(olProcessMethodElement(c_olProcessLegacy),
                            pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(olProcessMethodElement(c_olProcessPowerLawOnly),
                            pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(olProcessMethodElement(c_olProcessMoffatPeaks),
                            pcf::IndiElement::Off));

    REQUIRE(app.handleExtrapMethodPropertyForTest(ip) == 0);
    REQUIRE(app.extrapMethod() == c_olProcessMoffatPeaks);
    REQUIRE(app.extrapMethodElementStateForTest(olProcessMethodElement(
                c_olProcessMoffatPeaks)) == pcf::IndiElement::On);
    REQUIRE(app.extrapMethodElementStateForTest(olProcessMethodElement(
                c_olProcessLegacy)) == pcf::IndiElement::Off);
  }

  SECTION("noise-estimate domain is restored") {
    app.setExtrapNoiseEstimateDomainForTest(
        c_extrapNoiseEstimateClosedLoopPreXfer);

    pcf::IndiProperty ip(pcf::IndiProperty::Switch);
    ip.add(pcf::IndiElement(
        extrapNoiseEstimateDomainElement(c_extrapNoiseEstimateOpenLoop),
        pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(extrapNoiseEstimateDomainElement(
                                c_extrapNoiseEstimateClosedLoopPreXfer),
                            pcf::IndiElement::Off));
    ...
    REQUIRE(app.extrapNoiseEstimateDomainElementStateForTest(
                extrapNoiseEstimateDomainElement(
                    c_extrapNoiseEstimateOpenLoop)) == pcf::IndiElement::Off);
  }
  ...
  SECTION("noise-estimate range is restored") {
    ...
    REQUIRE(app.extrapNoiseEstimateRangeElementStateForTest(
                extrapNoiseEstimateRangeElement(
                    c_extrapNoiseEstimateLowFreq)) == pcf::IndiElement::On);
    REQUIRE(app.extrapNoiseEstimateRangeElementStateForTest(
                extrapNoiseEstimateRangeElement(
                    c_extrapNoiseEstimateHighFreq)) == pcf::IndiElement::Off);
  }
  ...
  SECTION("noise-estimate statistic is restored") {
    ...
    REQUIRE(app.extrapNoiseEstimateStatisticElementStateForTest(
                extrapNoiseEstimateStatisticElement(
                    c_extrapNoiseEstimateMinimum)) == pcf::IndiElement::On);
    REQUIRE(app.extrapNoiseEstimateStatisticElementStateForTest(
                extrapNoiseEstimateStatisticElement(
  ...
  SECTION("closed-loop OL estimate method is restored") {
    app.setExtrapClosedLoopOlEstimateMethodForTest(
        c_extrapClosedLoopOlEstimateNtfAware);

    pcf::IndiProperty ip(pcf::IndiProperty::Switch);
    ip.add(pcf::IndiElement(extrapClosedLoopOlEstimateMethodElement(
                                c_extrapClosedLoopOlEstimateEtfOnly),
                            pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(extrapClosedLoopOlEstimateMethodElement(
                                c_extrapClosedLoopOlEstimateNtfAware),
    ...
    REQUIRE(app.extrapClosedLoopOlEstimateMethod() ==
            c_extrapClosedLoopOlEstimateNtfAware);
    REQUIRE(app.extrapConfig().m_closedLoopOlEstimateMethod == "ntf-aware");
    REQUIRE(app.extrapClosedLoopOlEstimateMethodElementStateForTest(
                extrapClosedLoopOlEstimateMethodElement(
                    c_extrapClosedLoopOlEstimateNtfAware)) ==
            pcf::IndiElement::On);
    REQUIRE(app.extrapClosedLoopOlEstimateMethodElementStateForTest(
  ...
  SECTION("power-law crossover mode is restored") {
    app.setExtrapPowerLawCrossoverModeForTest(
        c_extrapPowerLawCrossoverAutoSmoothedCrossing);
    ...
    ip.add(pcf::IndiElement(
        extrapPowerLawCrossoverModeElement(c_extrapPowerLawCrossoverManual),
        pcf::IndiElement::Off));
    ip.add(pcf::IndiElement(extrapPowerLawCrossoverModeElement(
                                c_extrapPowerLawCrossoverAutoSmoothedCrossing),
                            pcf::IndiElement::Off));

    REQUIRE(app.handleExtrapPowerLawCrossoverModePropertyForTest(ip) == 0);
    REQUIRE(app.extrapPowerLawCrossoverMode() ==
            c_extrapPowerLawCrossoverAutoSmoothedCrossing);
    REQUIRE(app.extrapConfig().m_powerLawCrossoverMode ==
            "auto-smoothed-crossing");
    ...
    REQUIRE(app.extrapPowerLawCrossoverModeElementStateForTest(
                extrapPowerLawCrossoverModeElement(
                    c_extrapPowerLawCrossoverManual)) == pcf::IndiElement::Off);
  }
}
...
TEST_CASE("modalPsdProcessor falls back when the requested power-law fit has "
  ...
  mx::error_t errc = processPsdProcessorT::analyzePsd(result, measuredPsd, freq,
                                                      10, cfg, 0.0F, 25.0F);
...
TEST_CASE("modalPsdProcessor can estimate noise in closed-loop space before OL "
  ...
  std::vector<float> measuredPsd{0.0F, 5.0F, 5.0F, 1.0F,
                                 1.0F, 1.0F, 1.0F, 1.0F};
  ...
  mx::error_t errc = processPsdProcessorT::analyzePsd(
      result, measuredPsd, freq, 10, cfg, 0.0F, 25.0F, &correctionPsd);
...
TEST_CASE("modalPsdProcessor can estimate noise from the low-frequency end",
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F,  2.0F,  2.0F,  20.0F,
  ...
  mx::error_t errc = processPsdProcessorT::estimateNoisePsd(
      noisePsd, noiseFloor, measuredPsd, freq, 10, "low_freq");
...
TEST_CASE("modalPsdProcessor can estimate noise using the minimum PSD in range",
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F,  8.0F,  4.0F,  2.0F,
                                 20.0F, 20.0F, 20.0F, 20.0F};
  std::vector<float> freq{0.0F, 1.0F, 2.0F, 3.0F, 4.0F, 5.0F, 6.0F, 7.0F};
  std::vector<float> noisePsd;
  float noiseFloor = 0.0F;
  ...
  mx::error_t errc = processPsdProcessorT::estimateNoisePsd(
      noisePsd, noiseFloor, measuredPsd, freq, 10, "low_freq", "minimum");
...
TEST_CASE("modalPsdProcessor can limit low-frequency noise estimation to a max "
          "frequency",
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F,  20.0F, 20.0F, 2.0F,  2.0F,
                                 20.0F, 20.0F, 20.0F, 20.0F, 20.0F};
  std::vector<float> freq{0.0F, 1.0F, 2.0F, 3.0F, 4.0F,
                          5.0F, 6.0F, 7.0F, 8.0F, 9.0F};
  std::vector<float> noisePsd;
  float noiseFloor = 0.0F;

  mx::error_t errc = processPsdProcessorT::estimateNoisePsd(
      noisePsd, noiseFloor, measuredPsd, freq, 10, "low_freq", "percentile",
      2.1F);
...
TEST_CASE("modalPsdProcessor can reconstruct OL PSD with NTF-aware closed-loop "
  ...
  std::vector<float> measuredPsd{0.0F, 10.0F, 10.0F, 2.0F,
                                 2.0F, 2.0F,  2.0F,  2.0F};
  std::vector<float> freq{0.0F, 1.0F, 2.0F, 3.0F, 4.0F, 5.0F, 6.0F, 7.0F};
  ...
  processPsdProcessorT::processResults result;
  mx::error_t errc = processPsdProcessorT::analyzePsd(
      result, measuredPsd, freq, 10, cfg, 0.0F, 25.0F, &etfPsd, &ntfPsd);
...
TEST_CASE("modalPsdProcessor fits closed-loop noise on raw CL PSD even when OL "
  ...
  std::vector<float> measuredPsd{0.0F, 20.0F, 18.0F, 2.0F,
                                 2.0F, 2.0F,  2.0F,  2.0F};
  std::vector<float> freq{0.0F, 1.0F, 2.0F, 3.0F, 4.0F, 5.0F, 6.0F, 7.0F};
  std::vector<float> etfPsd(measuredPsd.size(), 0.5F);
...
TEST_CASE("modalPsdProcessor power-law-only matches moffat handoff when forced "
          ...
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F, 11.0F, 9.0F, 7.0F, 5.0F, 4.0F,
                                 3.4F, 3.0F,  2.8F, 2.6F, 2.4F};
  std::vector<float> freq{0.0F,   20.0F,  40.0F,  60.0F,  80.0F, 100.0F,
  ...
  mx::error_t errc = processPsdProcessorT::analyzePsd(powerResult, measuredPsd,
                                                      freq, 10, powerCfg);
  REQUIRE(!errc);

  errc = processPsdProcessorT::analyzePsd(moffatResult, measuredPsd, freq, 10,
                                          moffatCfg);
  REQUIRE(!errc);
  ...
  for (size_t n = 0; n < powerResult.m_processPsd.size(); ++n) {
    REQUIRE(powerResult.m_processPsd[n] ==
            Approx(moffatResult.m_processPsd[n]));
  }
}
...
TEST_CASE("modalPsdProcessor power-law-only repairs deep dropouts below the "
          ...
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F, 11.0F, 10.0F, 2.5F,
                                 9.0F, 8.0F,  7.5F,  7.0F};
  std::vector<float> freq{0.0F,  20.0F,  40.0F,  60.0F,
  ...
  REQUIRE(result.m_processPsd[5] ==
          Approx(result.m_extrapolation * pow(20.0F / freq[5], 1.0F)));
  REQUIRE(result.m_processPsd[6] ==
          Approx(result.m_extrapolation * pow(20.0F / freq[6], 1.0F)));
  REQUIRE(result.m_processPsd[7] ==
          Approx(result.m_extrapolation * pow(20.0F / freq[7], 1.0F)));
}

TEST_CASE("modalPsdProcessor can auto-select the power-law crossover from a "
          "smoothed noise crossing",
          "[modalGainOpt]") {
  std::vector<float> measuredPsd{0.0F, 10.0F, 8.5F, 6.5F,  4.5F, 3.1F,
                                 2.7F, 2.55F, 4.6F, 2.51F, 2.5F};
  std::vector<float> freq{0.0F,   20.0F,  40.0F,  60.0F,  80.0F, 100.0F,
                          120.0F, 140.0F, 160.0F, 180.0F, 200.0F};
  ...
  REQUIRE(result.m_processPsd[8] <
          static_cast<float>(measuredPsd[8] - result.m_noiseFloor));
}
...
TEST_CASE("modalPsdProcessor falls back to the highest-frequency smoothed "
  ...
  mx::error_t errc = processPsdProcessorHarness::findAutoPowerLawCrossoverFreq(
...
TEST_CASE(
  ...
  app.configurePublishedGainState(
      {2.0F, 4.0F}, {3.0F, 5.0F}, {7.0F, 11.0F}, {13.0F, 17.0F}, {19.0F, 23.0F},
      {1.0F, 2.0F}, {0.1F, 0.2F}, {1.1F, 1.2F}, {2.1F, 2.2F}, 2.0F);
  ...
  app.writePublishedGainArraysForTest(currentData.data(), siData.data(),
                                      maxSiData.data(), lpData.data(),
                                      maxLpData.data(), modeVarData.data());
 ...
 * \ingroup modalGainOpt_unit_test
 */
TEST_CASE("modalGainOpt published gain arrays apply calibration scaling",
          "[modalGainOpt]") {
  modalGainOptHarness app;

  app.configurePublishedGainState({6.0F, 3.0F}, {4.0F, 12.0F}, {8.0F, 18.0F},
                                  {10.0F, 20.0F}, {14.0F, 24.0F}, {3.0F, 6.0F},
                                  {0.4F, 0.8F}, {1.4F, 1.8F}, {2.4F, 2.8F},
                                  4.0F);
...
TEST_CASE(
  ...
  REQUIRE(app.countEnabledGainFactorsForTest(
              {-0.2F, 0.0F, 0.1F, 2.0F, -3.0F}) == 2);
...
TEST_CASE("modalGainOpt gain-factor updates leave state unchanged when the "
  ...
  REQUIRE(app.applyGainFactorUpdateForTest(storedGainFacts, {1.0F, 0.5F, 0.0F},
                                           false) == false);
...
TEST_CASE("modalGainOpt gain-factor updates resize SI state and reset "
  ...
  REQUIRE(app.applyGainFactorUpdateForTest(
              storedGainFacts, {0.0F, 1.5F, -2.0F, 3.0F}, false) == true);
...
TEST_CASE("modalGainOpt multiplier updates leave state unchanged when the "
  ...
  REQUIRE(app.applyMultiplierUpdateForTest(
              storedMultFacts, {0.25F, 0.5F, 0.75F}, false) == false);
...
TEST_CASE(
  ...
  REQUIRE(app.applyMultiplierUpdateForTest(storedMultFacts, {1.5F, 2.5F},
                                           false) == true);
 ...
 * \ingroup modalGainOpt_unit_test
 */
TEST_CASE("modalGainOpt refreshes pending gopt structures without a PSD wakeup",
          "[modalGainOpt]") {
  modalGainOptHarness app;
  ...
  app.configureGoptStructureInputsForTest({1.0F, 2.0F}, {0.001F, 0.002F},
                                          {0.5F, 0.75F}, app.freq());
...
TEST_CASE(
  ...
  app.configurePublishedPredictorState(
      {2.0F, 4.0F}, {3.0F, 5.0F}, {1.0F, 2.0F}, {2U, 1U}, {1U, 2U},
      {{0.1F, 0.2F}, {0.4F}}, {{0.3F}, {0.5F, 0.6F}}, 2.0F, 0.5F);

  std::vector<float> pcGainData(2, -1.0F);
  std::vector<float> aCoeffData(8, -1.0F);
  std::vector<float> bCoeffData(8, -1.0F);

  app.writePublishedPredictorArraysForTest(pcGainData.data(), aCoeffData.data(),
                                           4, bCoeffData.data(), 4, false);
...
TEST_CASE("modalGainOpt predictor publication blends existing values and "
  ...
  app.configurePublishedPredictorState(
      {2.0F, 4.0F}, {3.0F, 5.0F}, {1.0F, 2.0F}, {2U, 1U}, {1U, 2U},
      {{0.1F, 0.5F}, {0.9F}}, {{0.2F}, {0.6F, 1.0F}}, 2.0F, 0.25F);

  std::vector<float> pcGainData({1.0F, 9.0F});
  std::vector<float> aCoeffData(
      {9.0F, 1.0F, 2.0F, 3.0F, 8.0F, 4.0F, 5.0F, 6.0F});
  std::vector<float> bCoeffData(
      {7.0F, 1.0F, 2.0F, 3.0F, 6.0F, 4.0F, 5.0F, 6.0F});

  app.writePublishedPredictorArraysForTest(pcGainData.data(), aCoeffData.data(),
                                           4, bCoeffData.data(), 4, true);
...
TEST_CASE("modalGainOpt predictor publication clears stale coefficient blocks "
  ...
  app.configurePublishedPredictorState({8.0F, 2.0F}, {4.0F, 6.0F}, {4.0F, 2.0F},
                                       {0U, 0U}, {0U, 0U}, {{}, {}}, {{}, {}},
                                       2.0F, 0.5F);
  ...
  std::vector<float> aCoeffData(
      {7.0F, 1.0F, 2.0F, 3.0F, 6.0F, 4.0F, 5.0F, 6.0F});
  std::vector<float> bCoeffData(
      {5.0F, 7.0F, 8.0F, 9.0F, 4.0F, 10.0F, 11.0F, 12.0F});
...
TEST_CASE("modalGainOpt PC gain-factor updates preserve applied mode counts "
  ...
  REQUIRE(app.applyGainFactorUpdateForTest(storedPcGainFacts,
...
TEST_CASE("modalGainOpt PC multiplier updates set pcgoptUpdated without "
  ...
  REQUIRE(app.applyMultiplierUpdateForTest(storedPcMultFacts,
                                           {0.3F, 0.4F, 0.5F}, true) == true);
