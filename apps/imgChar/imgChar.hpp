...
int imgChar::appLogic()
{
   ...
   {
      m_xshiftRMS  += x_1sec[i];
      m_yshiftRMS  += y_1sec[i];
      m_strehlMean += s_1sec[i];
      m_strehlRMS  += s_1sec[i] * s_1sec[i];
   }

   m_xshiftRMS  /= (size_t)fps();
   m_yshiftRMS  /= (size_t)fps();
   m_strehlMean /= (size_t)fps();
   m_strehlRMS  /= (size_t)fps();

   x_10_1s[idx_10buf]  = m_xshiftRMS;
   y_10_1s[idx_10buf]  = m_yshiftRMS;
   s_10_1sR[idx_10buf] = m_strehlRMS;
   s_10_1sM[idx_10buf] = m_strehlMean;
   ...
   m_xshiftRMS = sqrt(m_xshiftRMS);
   m_yshiftRMS = sqrt(m_yshiftRMS);
   m_strehlRMS = sqrt(m_strehlRMS);
