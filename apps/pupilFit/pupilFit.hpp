...
inline
pupilFit::pupilFit() : MagAOXApp(MAGAOX_GIT_SHA1, MAGAOX_GIT_MODIFIED, MAGAOX_GIT_URL, MAGAOX_GIT_BRANCH, MAGAOX_SOURCE_PATH, MAGAOX_GIT_UNTRACKED)
{
...
int pupilFit::appStartup()
{
   ...
   indi::addNumberElement<float>( m_indiP_quad1, "bg", 0, std::numeric_limits<uint16_t>::max(), 0, "%0.1f", "background");
   ...
   indi::addNumberElement<float>( m_indiP_quad2, "bg", 0, std::numeric_limits<uint16_t>::max(), 0, "%0.1f", "background");
   ...
   indi::addNumberElement<float>( m_indiP_quad3, "bg", 0, std::numeric_limits<uint16_t>::max(), 0, "%0.1f", "background");
   ...
   if(m_numPupils != 3)
   {
      ...
      indi::addNumberElement<float>( m_indiP_quad4, "bg", 0, std::numeric_limits<uint16_t>::max(), 0, "%0.1f", "background");
                          ...
                          )
{
   ...
   {//mutex scope
      ...
      m_indiP_quad1["bg"].set(m_fitter.m_bg[0]);
      ...
      m_indiP_quad2["bg"].set(m_fitter.m_bg[1]);
      ...
      m_indiP_quad3["bg"].set(m_fitter.m_bg[2]);
      ...
      else
      {
         ...
         m_indiP_quad4["bg"].set(m_fitter.m_bg[3]);
