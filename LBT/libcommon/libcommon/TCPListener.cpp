...
int TCPListener::listen( const int &nPort,
                         TCPConnection *pConnectionPrototype,
                         const string &szHost )
{
  Logger logMsg;
  int nErr = enumNoError;
  ...
  try
  {
    if ( m_oIsListening != true )
    {
      ...
      if ( ( nErr = m_socServer.create() ) == SystemSocket::enumNoError )
      {
        //  bind it to the port supplied....
        if ( ( nErr = m_socServer.bind ( nPort, szHost ) )
             != SystemSocket::enumNoError )
        {
          nErr = enumBindFailed;
        }
        else
        {
          //m_socServer.setNonBlocking( true );

          //  spawn a thread to listen for new connections.
          if ( ( nErr = start() ) != Thread::ErrNone )
          {
            nErr = enumStartFailed;
          }
        }
      }
    }
...
void TCPListener::beforeExecute()
{
  ...
  if ( ( nErr = m_socServer.listen() ) == SystemSocket::enumNoError )
  {
    m_oIsListening = true;
    Logger logMsg;
...
void TCPListener::execute()
{
  ...
  if ( m_oIsListening == true )
  {
    ...
    nErr = m_socServer.accept( m_pConnection->getSocket() );
    if ( nErr == SystemSocket::enumNoError )
    {
      Logger logMsg;
