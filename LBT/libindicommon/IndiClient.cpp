...
void IndiClient::setup()
{
  ...
  {
    Config cfReader;
    string szHostIp = cfReader.get<string>( "indi_server_ip", "127.0.0.1" );
    int szHostPort = cfReader.get<int>( "indi_server_port", 9752 );

    logMsg << Logger::Info << "    Read & set the client configuration." << endl;

    m_socClient.create ( SystemSocket::enumStream );

    int iErr = SystemSocket::enumNoError;
    if ( ( iErr = m_socClient.connect( szHostPort, szHostIp ) )
         != SystemSocket::enumNoError )
    {
      logMsg << Logger::enumError
             << "Cannot connect to " << szHostIp << ":" << szHostPort << " '"
             << SystemSocket::getErrorMsg( iErr ) << "'." << endl;
      m_socClient.close();
      throw runtime_error( SystemSocket::getErrorMsg( iErr ) );
    }
    else
    {
      logMsg << Logger::enumInfo
             << "    Connected to server at " << szHostIp << ":" << szHostPort << endl;
