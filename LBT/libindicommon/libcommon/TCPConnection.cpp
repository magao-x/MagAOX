...
void TCPConnection::execute()
{
  ...
  {
    string szData;
    int nStatus = m_socConnection.recv( szData );
    ...
    if ( nStatus == SystemSocket::enumWouldBlock )
    {
      Thread::msleep( 1 );
    }
    //  did the socket close?
    else if ( nStatus == SystemSocket::enumPeerShutdown )
    {
      logMsg << Logger::enumInfo << "OK      [TCPConnection::execute] "
             << "(fd=" << m_socConnection.getFd() << ") "
             << SystemSocket::getErrorMsg( nStatus )
             << endl;
      stop();
    }
  ...
  else
  {
    ...
    int nStatus = m_socConnection.recvChunk( pcData, nSize );
...
int TCPConnection::sendChunk( const char *pcData, int &nSize  )
{
  ...
  return m_socConnection.sendChunk( const_cast<char *>( pcData ), nSize );
}
