...
int ConfigFile::readFile()
{
  ...
  while ( ifs.getline( pcLine, 1024 ).fail() == false )
  {
    ...
    // Do we have a group? This may be an entry with an empty value, though.
    else if ( vecEntry.size() == 1 )
    {
      int nLength = vecEntry[0].length();

      // If the name is less than two chars, it can't be a group.
      // Is it properly surrounded by square braces? If not, we have an
      // empty entry and not a group name.
      if ( nLength <= 2 ||
      ...
      {
        string szNewGroup = vecEntry[0];
        szNewGroup[0] = szNewGroup[nLength-1] = ' ';
        szNewGroup = Cmd::trimWhitespace( szNewGroup );

        // Do we have a valid name?
        if ( szNewGroup.length() == 0 )
        {
          throw runtime_error( string( "Config: Empty group name" ) +
              " (line #" + ssCurrLine.str() + ")" );
        }
        ...
        // Make sure the 'group close' matches the 'group open'
        if ( oInGroup == false && szNewGroup != szGroup )
        {
          throw runtime_error( string( "Config: Wrong group name at close" ) +
              " (line #" + ssCurrLine.str() + ")" );
        }
    ...
    //  is there a standard group entry (name-value pair)?
    else if ( vecEntry.size() >= 2 && oInGroup == true )
    {
      m_mapVars[ szGroup + "." + vecEntry[0] ] = vecEntry[1];
    }

    //  is there a standard non-group entry (name-value pair)?
    else if ( vecEntry.size() >= 2 && oInGroup == false )
    {
...
vector<string> ConfigFile::getFileList( string &szPath, string &szExtFilter )
{
  vector<string> vecFiles;
  size_t tSize = szExtFilter.size();

  DIR *dirFile = ::opendir( szPath.c_str() );
  if ( dirFile != NULL )
  {
    struct dirent *hFile;
    while ( ( hFile = ::readdir( dirFile ) ) != NULL )
    {
      string szName( hFile->d_name );

      // Ignore hidden files, current dir, parent dir and
      // make sure it matches the extension filter.
      if ( szName[0] != '.' && szName != "." && szName != ".." &&
           tSize < szName.size() &&
           szName.substr( szName.size() - tSize ) == szExtFilter )
      {
        vecFiles.push_back( szName );
      }
