...
/// @param szName The property name.

IndiPropertyMap::Iterator IndiPropertyMap::find( const string& szName )
{
  return m_mapProperties.find( szName );
}
...
IndiPropertyMap::ConstIterator IndiPropertyMap::find( const string& szName ) const
{
  return m_mapProperties.find( szName );
}
...
const IndiProperty &IndiPropertyMap::operator[] ( const string& szName ) const
{
  return m_mapProperties.find(szName)->second;
}

////////////////////////////////////////////////////////////////////////////////
// Return a reference to a property so it can be modified.

IndiProperty &IndiPropertyMap::operator[] ( const string& szName )
{
  return m_mapProperties.find(szName)->second;
}
...
///
void IndiPropertyMap::erase( const string &szName )
{
  m_mapProperties.erase( szName );
}
                               ...
                               const string &szNewName ) const
{
  try
  {
    IndiProperty ipSend = find( szPropertyName )->second;
  ...
  {
    throw runtime_error( string( "Couldn't find property '" ) +
                         szPropertyName + "'" );
  }
}
...
void IndiPropertyMap::sendSetForAll( const IndiDriver &idParent,
                                     const string &szNewName ) const
{
  map<string, IndiProperty>::const_iterator itr = m_mapProperties.begin();
  for ( ; itr != m_mapProperties.end(); itr++ )
...
void IndiPropertyMap::updateAllStates( const IndiDriver &idParent,
                                       const IndiProperty::PropertyStateType &pstNew,
                                       const string &szNewName )
{
  map<string, IndiProperty>::iterator itr = m_mapProperties.begin();
...
/// @param idDriver Parent Driver instance.

void IndiPropertyMap::sendGet( IndiDriver &idParent ) const
{
  // Issue a get property for each
  map<string, IndiProperty>::const_iterator itr = m_mapProperties.begin();
  for ( ; itr != m_mapProperties.end(); ++itr)
  {
