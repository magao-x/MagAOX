...
class statusEntry : public QWidget, public multiIndiSubscriber
{
  ...
  public:
    statusEntry( QWidget *Parent = 0, Qt::WindowFlags f = Qt::WindowFlags() );
    ...
    QString formattedValue( const std::string &value );
    ...
    /// Set the show target flag
    /** If set to false, the widget will not show the target value in parentheses.
     * The default is fase.
     *
     * This sets m_showTarget
     */
    void showTarget( bool st );

    /// Get the value of the show target flag
    /**
     * \returns the current value of m_showTarget
     */
    bool showTarget();
...
statusEntry::statusEntry( QWidget *Parent, Qt::WindowFlags f ) : QWidget( Parent, f )
{
    construct();
}
...
QString statusEntry::formattedValue( const std::string &value )
{
    char str[64];
    switch( m_type )
    {
    ...
    case INT:
        try
        {
            snprintf( str, sizeof( str ), m_format.c_str(), std::stoi( value ) );
            return QString( str );
        }
        catch( ... )
        {
    ...
    case FLOAT:
        if( m_format == "auto" )
        {
            return floatAutoFormat( value );
        }

        try
        {
            snprintf( str, sizeof( str ), m_format.c_str(), std::stof( value ) );
            return QString( str );
        }
        catch( ... )
        {
...
void statusEntry::showTarget( bool st )
{
    m_showTarget = st;
}

bool statusEntry::showTarget()
{
    return m_showTarget;
}
...
void statusEntry::setStretch( int s0, int s1, int s2 )
{
...
void statusEntry::updateGUI()
{
    ...
    {
        QString ntxt;
        if( m_showTarget && ( m_target != m_current ) )
        {
            ntxt = QString( "%1 (%2)" ).arg( formattedValue( m_current ), formattedValue( m_target ) );
        }
        else
        {
            ntxt = formattedValue( m_current );
        }

        if( m_valChanged )
        {
            ui.value->setTextChanged( ntxt );

            m_valChanged = false;
        }
        else
        {
            ui.value->setText( ntxt );
        }
    }
    else
    {
        ui.value->setDisabledText();
    }
