...
int pwfsAlignment::updateOverlay()
{
   ...
   char * str;
   if( m_dict->count(m_deviceName + ".numPupils.value") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".numPupils.value"].m_blob;
      m_numPupils = atoi(str);
   }
   
   if( m_dict->count(m_deviceName + ".sm_frameSize.width") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".sm_frameSize.width"].m_blob;
      m_width = atoi(str);
   }
   
   if( m_dict->count(m_deviceName + ".sm_frameSize.height") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".sm_frameSize.height"].m_blob;
      m_height = atoi(str);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant1.x") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant1.x"].m_blob;
      m_1x = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant1.y") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant1.y"].m_blob;
      m_1y = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant1.D") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant1.D"].m_blob;
      m_1D = strtod(str,0);
   }
   //std::cerr << "1: " << m_1x << " " << m_1y << " " << m_1D << "\n";
   
   
   if( m_dict->count(m_deviceName + ".quadrant2.x") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant2.x"].m_blob;
      m_2x = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant2.y") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant2.y"].m_blob;
      m_2y = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant2.D") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant2.D"].m_blob;
      m_2D = strtod(str,0);
   }
   //std::cerr << "2: " <<  m_2x << " " << m_2y << " " << m_2D << "\n";
   
   if( m_dict->count(m_deviceName + ".quadrant3.x") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant3.x"].m_blob;
      m_3x = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant3.y") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant3.y"].m_blob;
      m_3y = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant3.D") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant3.D"].m_blob;
      m_3D = strtod(str,0);
   }
   //std::cerr << "3: " <<  m_3x << " " << m_3y << " " << m_3D << "\n";
   
   
   if( m_dict->count(m_deviceName + ".quadrant4.x") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant4.x"].m_blob;
      m_4x = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant4.y") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant4.y"].m_blob;
      m_4y = strtod(str,0);
   }
   
   if( m_dict->count(m_deviceName + ".quadrant4.D") > 0)
   {
      str = (char *)(*m_dict)[m_deviceName + ".quadrant4.D"].m_blob;
      m_4D = strtod(str,0);
   }
   ...
   if(!initialized)
   {
      ...
      float dIw = 0.5*(m_width-1.0);
      float dQw = 0.5*(0.5*m_width - 1.0);
      
      float dIh = 0.5*(m_height-1.0);
      ...
      if(m_numPupils == 3)
      {
         m_1to2 = qgs->addLine(QLineF(dQw,(m_height-1)-dQh, 0.5*m_width+dQw, (m_height-1)-dQh ), QColor("red"));
         m_1to3 = qgs->addLine(QLineF(dQw,(m_height-1)-dQh, 0.5*m_height, dQh ), QColor("red"));
         m_3to4 = qgs->addLine(QLineF(0.5*m_height, dQh, 0.5*m_width+dQw, (m_height-1)-dQh), QColor("red"));
         
         m_c1 = qgs->addEllipse(dQw-14,(m_height-1)-dQh-14, 28.0, 28.0, QColor("red"));
         m_c2 = qgs->addEllipse(0.5*m_width+dQw-14, (m_height-1)-dQh-14, 28.0, 28.0, QColor("red"));
         m_c3 = qgs->addEllipse(0.5*m_height-14, dQh-14, 28.0, 28.0, QColor("red"));
