...
startLocalDvr (DvrInfo *dp)
{
	...
	if (!strcat_varargs(bnbuf, 2048, dp->name, NULL)) {
	    Bye ("Driver %s device name too long: %s\n", dp->name, strerror(errno));
	}
	pbn = basename(bnbuf);
	if (!pbn || !*pbn) {
	    Bye ("Driver %s device has no base name: %s; pbn='%s'\n", dp->name, strerror(errno), pbn ? pbn : NULLC);
	}

	/* build three pipes: r, w and error */
	if (pipe_wrapper (rp, "*INDIS_NAMED_FIFO_DIR", "/indid_stdout.", pbn) < 0)
	    Bye ("Driver %s read pipe: %s\n", dp->name, strerror(errno));
	if (pipe_wrapper (wp, "*INDIS_NAMED_FIFO_DIR", "/indid_stdin.", pbn) < 0)
	    Bye ("Driver %s write pipe: %s\n", dp->name, strerror(errno));
	if (pipe_wrapper (ep, "*INDIS_NAMED_FIFO_DIR", "/indid_stderr.", pbn) < 0)
