...
/* return the embedded XMLEle in the given LilXML.
 * returns NULL if lp does not yet point to a complete XMLEle or we are passed NULL
 */
XMLEle *
getXMLEle (LilXML *lp)
{
...
/* return a deep copy of the given LilXML
 */
LilXML *
cloneLilXML (LilXML *lp)
{
  LilXML *newlp = newLilXML();
  newlp->ce = cloneXMLEle (lp->ce);
  return (newlp);
}
